...
export default {
	...
	func: async (_req, res) => {
		...
		if (_req.method === 'GET') {
			a.get('https://gameinternationalization.roblox.com' + _req.url, {
				headers: { ..._req.headers, host: 'gameinternationalization.roblox.com' },
			})
				.then((re) => {
			...
			a.patch('https://gameinternationalization.roblox.com' + _req.url, _req.body, {
				headers: { ..._req.headers, host: 'gameinternationalization.roblox.com' },
			})
				.then((re) => {
					const newheaders = JSON.parse(JSON.stringify(re.headers).split('roblox.com').join('sitetest4.robloxlabs.com'));
