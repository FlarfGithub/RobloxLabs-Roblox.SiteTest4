...
export default {
	...
	func: async (_req, res) => {
		...
		if (_req.method === 'GET') {
			a.get('https://catalog.roblox.com' + _req.url, {
				headers: { ..._req.headers, host: 'catalog.roblox.com' },
			})
				.then((re) => {
			...
			a.delete('https://catalog.roblox.com' + _req.url, {
				headers: { ..._req.headers, host: 'catalog.roblox.com' },
			})
				.then((re) => {
					const newheaders = JSON.parse(JSON.stringify(re.headers).split('roblox.com').join('sitetest4.robloxlabs.com'));
			...
			a.post('https://catalog.roblox.com' + _req.url, _req.body, {
				headers: { ..._req.headers, host: 'catalog.roblox.com' },
			})
				.then((re) => {
					const newheaders = JSON.parse(JSON.stringify(re.headers).split('roblox.com').join('sitetest4.robloxlabs.com'));
