...
(ChatBar.CurrentTabID = null),
	...
	(ChatBar.DisplayLocalMessageInWindow = function (n, t) {
		$('#' + n + '_friend_dock_chatbox_chat').append(
			...
			$('#' + n + '_friend_dock_chatbox_chat').prop({
...
(function (c) {
	...
	c.jPlayer.convertTime = function (a) {
		...
		return (
			(c.jPlayer.timeFormat.showHour ? d + c.jPlayer.timeFormat.sepHour : '') +
			(c.jPlayer.timeFormat.showMin ? a + c.jPlayer.timeFormat.sepMin : '') +
	...
	c.jPlayer.prototype = {
		_init: function () {
			...
			c.extend(this.config, {
				canPlayMP3: !!(this.config.audio.canPlayType
					? '' != this.config.audio.canPlayType('audio/mpeg') && 'no' != this.config.audio.canPlayType('audio/mpeg')
					: false),
				canPlayOGG: !!(this.config.audio.canPlayType
					? '' != this.config.audio.canPlayType('audio/ogg') && 'no' != this.config.audio.canPlayType('audio/ogg')
					: false),
						...
						if (a.config.isFileSet)
							try {
								...
								if (typeof a.config.audio.buffered == 'object' && a.config.audio.buffered.length > 0)
									a.config.audio.currentTime =
										(f * a.config.audio.buffered.end(a.config.audio.buffered.length - 1)) / 100;
				...
				if (this._checkForFlash(8))
					if (c.browser.msie) {
						...
						d[2] =
							...
							escape(this.config.id) +
							'&fid=' +
							escape(this.config.fid) +
							'&vol=' +
							this.config.volume +
							'" />';
					...
					this.element.html(
						"<p>Flash 8 or above is not installed. <a href='http://get.adobe.com/flashplayer'>Get Flash!</a></p>",
					);
			else {
				this.config.audio.autobuffer = this.config.autobuffer;
				this.config.audio.preload = this.config.preload;
				this.config.audio.addEventListener(
					'canplay',
					function () {
			...
			this.config.customCssIds ||
				c.each(c.jPlayer._cssId, function (g, f) {
					a.cssId(g, f);
				});
			if (!this.config.usingFlash) {
		...
		jPlayerReady: function (a) {
			if (this.config.usingFlash) {
				...
				this.config.swfVersionRequired != this.config.swfVersion &&
					this._error(
						"jPlayer's JavaScript / SWF version mismatch!\n\nJavaScript requires SWF : " +
							this.config.swfVersionRequired +
							'\nThe Jplayer.swf used is : ' +
