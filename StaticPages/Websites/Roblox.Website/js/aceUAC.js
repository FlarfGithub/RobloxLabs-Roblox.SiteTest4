(function () {
	...
	var appendStaticIframe = function (options) {
		...
		fif.src =
			(document.location.protocol === 'https:' ? ACE3_UAC_CDN_ORIGIN_SECURE : ACE3_UAC_CDN_ORIGIN) +
			'/wrapper/aceUAC.htm' +
			objectToHash(options);
	...
	var ACE3CkPlg = function () {
		...
		if (ACE3IE && ua.indexOf('win') !== -1) {
			try {
				...
				ACE3FVer = new ActiveXObject('ShockwaveFlash.ShockwaveFlash')
					.GetVariable('$version')
					.replace(/\D+/g, ',')
					.match(/^,?(.+),?$/)[1]
					.split(',')
	...
	var hashPollFifPing = function (fifFrame, options, timeout, interval) {
		...
		var fifPoll = function () {
			...
			if (location.hash.match(/#fifgood/gi)) {
				success();
			} else if (location.hash.match(/#crossdomain/gi)) {
				fail();
			} else {
				if (diff > timeout) {
					fail();
	...
	var ACErtbCheck = function (site, region) {
		if (!document.getElementById('ACE3RTBPingFile') && Math.floor(Math.random() * 100) + 1 <= ACE3RTBglobalThrottle) {
			...
			rtbPing.src =
				(document.location.protocol === 'https:' ? ACE3SecureUserMatchingHost : ACE3UserMatchingHost) +
				'/um.ashx?site=' +
				site +
				rcode;
	...
	window['ACE3_AdRequest'] = function (obj) {
		...
		ACE3AllowExp =
			obj['allowexp'] == '0' || ex.indexOf('/aolexp=0') >= 0 ? 0 : obj['allowexp'] == '1' || ex.indexOf('/aolexp=1') >= 0 ? 2 : 1;
