...
Roblox.PlaceProductPromotion = (function () {
	...
	function e(n, t) {
		...
		$.ajax({
			type: 'GET',
			url:
				'/PlaceItem/GetPlaceProductPromotions?placeId=' +
				Roblox.PlaceProductPromotion.PlaceID +
				'&startIndex=' +
				n +
				'&maxRows=' +
				t +
	...
	function f() {
		...
		var i = $('#PlaceProductPromotionInput').val(),
			n;
		$('.PromoteModalErrorMessage').hide(),
			(n = $('#PlaceProductPromotionSelectionPanel')),
			$.ajax({
				type: 'POST',
				...
				success: function (t) {
					if (t.ErrorMsg) n.find('.PromoteModalErrorMessage').text(t.ErrorMsg).show();
						...
						var i = $(
							'<div>' +
								Roblox.Resources.PlaceProductPromotion.youhaveAdded +
								'<span></span>' +
								Roblox.Resources.PlaceProductPromotion.toYourGame +
								'<a></a></div>',
						);
						i.find('span').html(Roblox.PlaceProductPromotionData.ProductName),
							i.find('a').attr('href', t.PlaceURL).text(t.PlaceName),
							n.find('.roblox-item-image').html('').addClass('thumbs-up-green'),
							n.find('.PurchaseModalMessageText').html(i).addClass('SuccessMsg'),
							n.find('.PurchaseModalButtonContainer .promoteBtn').hide(),
							n.find('.titleBar').text(Roblox.Resources.PlaceProductPromotion.success),
							n.find('.PurchaseModalFooter').text(''),
							n
								.find('.PurchaseModalButtonContainer .btn-negative')
								.removeClass('btn-negative')
								.addClass('btn-neutral')
								.html(
									Roblox.Resources.PlaceProductPromotion.ok +
										'<span class="btn-text">' +
										Roblox.Resources.PlaceProductPromotion.ok +
										'</span>',
								);
	...
	function n(n) {
		$.ajax({
			...
			data: o({}),
			success: function () {
				var i = $('.DeleteProductPromotion[data-delete-promotion-id="' + n + '"]')
					.parent()
					.find('.NameDiv')
					.html();
				$('#DeleteProductPromotionModal .titleBar').text(Roblox.Resources.PlaceProductPromotion.success),
					$('#DeleteProductPromotionModal .PurchaseModalMessageText').html(
						Roblox.Resources.PlaceProductPromotion.youhaveRemoved +
							'<a>' +
							i +
							'</a>' +
							Roblox.Resources.PlaceProductPromotion.fromYourGame,
					),
					$('#DeleteProductPromotionModal').modal(Roblox.PlaceProductPromotion.modalProperties),
					Roblox.PlaceProductPromotionPager.getItemsPaged(1, 3);
			},
			error: function () {
				$('#DeleteProductPromotionModal .titleBar').text(Roblox.Resources.PlaceProductPromotion.error),
					$('#DeleteProductPromotionModal .PurchaseModalMessageText').text(Roblox.Resources.PlaceProductPromotion.sorryWeCouldnt),
					$('#DeleteProductPromotionModal').modal(Roblox.PlaceProductPromotion.modalProperties);
			},
		});
	}
	function r(n) {
		...
		return (
			t.find('.ItemURL').attr('href', n.ItemUrl).html(fitStringToWidthSafe(n.Name, 185)),
			t.find('.TotalSales').text(n.TotalSales),
			t.find('.DeleteProductPromotion').attr('data-delete-promotion-id', n.PromotionID),
			n.PriceInRobux != undefined
				? (t.find('.Price').text(n.PriceInRobux),
				  t.find('.PurchaseButton').attr('data-expected-price', n.PriceInRobux).attr('data-promotion-id', n.PromotionID))
				: n.PriceInTickets != undefined
				? (t.find('.Price').removeClass('robux').addClass('tickets').text(n.PriceInTickets),
				  t
						.find('.PurchaseButton')
						.attr('data-expected-currency', 2)
						.removeAttr('data-promotion-id')
						.attr('data-expected-price', n.PriceInTickets)
						.removeAttr('data-placeproductpromotion-id'))
				: t.find('.Price').remove(),
			n.IsForSale ||
				(t.find('.PurchaseButton').hide(), t.find('.UserOwns').html(Roblox.Resources.PlaceProductPromotion.notForSale).show()),
			t
				.find('.PurchaseButton')
				.attr('data-expected-seller-id', n.SellerID)
				.attr('data-seller-name', n.SellerName)
				.attr('data-item-name', n.Name),
			n.UserOwns === !0 && (t.find('.PurchaseButton').hide(), t.find('.UserOwns').show()),
			n.IsRentable === !0 &&
				t
					.find('.PurchaseButton')
					.html(
						Roblox.Resources.PlaceProductPromotion.rent +
							'<span class=btn-text>' +
							Roblox.Resources.PlaceProductPromotion.rent +
							'</span>',
					),
			t.find('[data-bc-requirement]').attr('data-bc-requirement', n.BCRequirement),
			t.find('[data-item-id]').attr('data-item-id', n.AssetID),
			t.find('[data-product-id]').attr('data-product-id', n.ProductID),
			t.find('[data-placeproductpromotion-id]').attr('data-placeproductpromotion-id', n.PromotionID),
			t
		);
	}
	function i(n) {
	...
	function t(n) {
		...
		$.each(n.UserPlaces, function (n, i) {
			...
			$('.PromoteModalBtn').click(function () {
				var n = { escClose: !0, opacity: 80, overlayCss: { backgroundColor: '#000' } };
				$('#PlaceProductPromotionSelectionPanel').modal(n),
					Roblox.require('Widgets.ItemImage', function (n) {
	...
	var s,
		u = { escClose: !0, opacity: 80, overlayCss: { backgroundColor: '#000' } };
	return {
...
Roblox.Voting = (function () {
	var i = function (n) {
		...
		u = function (n, t, r) {
			$('.voting-panel .loading').show(),
				$.ajax({
					type: 'POST',
					url: '/place-item/vote?assetId=' + n + '&vote=' + t,
					data: i({}),
					success: function (n) {
						$('.voting').html(n), r();
					},
		...
		f = function (n, t) {
			$('.voting-panel .loading').show(),
				$.ajax({
					type: 'POST',
					url: '/place-item/unvote?assetId=' + n,
					data: i({}),
					success: function (n) {
						$('.voting').html(n), t();
					},
		...
		t = function (t, i) {
			var u = $('.voting-panel').attr('data-vote-modal'),
				r;
			if (u != undefined && u.length > 0)
				...
				(r = $('.voting-panel').attr('asset-id')),
					t.hasClass('selected') ? Roblox.Voting.Unvote(r, n) : Roblox.Voting.Vote(r, i, n);
		},
		n = function () {
			$('.users-vote .upvote')
				.unbind()
			...
			var i = parseInt($('.voting-panel').attr('total-up-votes')),
				r = parseInt($('.voting-panel').attr('total-down-votes')),
				n;
			isNaN(i) ||
				isNaN(r) ||
				((n = i === 0 ? 0 : r === 0 ? 100 : Math.floor((i / (i + r)) * 100)),
				n > 100 && (n = 100),
				$('.voting-panel .visual-container .percent').css('width', n + '%')),
				$('#VisitButtonContainer .btn-primary').click(function () {
					setTimeout(Roblox.Voting.CanVote, 1e4);
				});
		},
		r = function () {
			var n = $('.voting-panel').attr('asset-id');
			$.ajax({
				type: 'GET',
				url: '/place-item/canvote?assetId=' + n,
				success: function (n) {
					n.success
						? $('.voting-panel').removeAttr('data-vote-modal')
						: n.message === 'PlayGame' && setTimeout(Roblox.Voting.CanVote, 1e4);
				},
				error: function () {
					setTimeout(Roblox.Voting.CanVote, 1e4);
				},
	...
	return { Vote: u, Unvote: f, Initialize: n, CanVote: r };
...
(function (n) {
	function l(n, i, r) {
		var e = n.children(),
			...
			o,
			f;
		for (n.empty(), u = 0, o = e.length; o > u; u++) {
			if (((f = e.eq(u)), n.append(f), r && n.append(r), t(n, i))) {
				f.remove(), (s = !0);
				break;
			}
			r && r.remove();
	...
	function e(i, r, u, f, o) {
		...
		for (
			...
			(h = v[a]),
				...
				void 0 !== h &&
					(i.append(l),
					o && i[i.is(y) ? 'after' : 'append'](o),
					3 == h.nodeType ? t(u, f) && (s = c(l, r, u, f, o)) : (s = e(l, r, u, f, o)),
					s || (o && o.remove()));
		return s;
	}
	function c(n, i, u, f, e) {
		var y = !1,
			o = n[0],
			k,
			a,
		...
		for (var d = 'letter' == f.wrap ? '' : ' ', v = s(o).split(d), w = -1, l = -1, g = 0, nt = v.length - 1; nt >= g; ) {
			if (((k = Math.floor((g + nt) / 2)), k == l)) break;
			(l = k), r(o, v.slice(0, l + 1).join(d) + f.ellipsis), t(u, f) ? (nt = l) : ((w = l), (g = l));
		}
		return (
			-1 == w || (1 == v.length && 0 == v[0].length)
				? ((a = n.parent()),
				  n.remove(),
				  (b = e ? e.length : 0),
				  a.contents().size() > b
						? ((tt = a.contents().eq(-1 - b)), (y = c(tt, i, u, f, e)))
						: ((o = a.prev().contents().eq(-1)[0]), o !== void 0 && ((p = h(s(o), f)), r(o, p), a.remove(), (y = !0))))
				: ((p = h(v.slice(0, w + 1).join(d), f)), (y = !0), r(o, p)),
			y
		);
	}
	function t(n, t) {
	...
	function v(n) {
		for (var t, u = n.innerHeight(), r = ['paddingTop', 'paddingBottom'], i = 0, f = r.length; f > i; i++)
			(t = parseInt(n.css(r[i]), 10)), isNaN(t) && (t = 0), (u -= t);
		return u;
	}
	function y(n, t) {
		return n
			? ((t = 'string' == typeof t ? 'dotdotdot: ' + t : ['dotdotdot:', t]),
			  window.console !== void 0 && window.console.log !== void 0 && window.console.log(t),
			  !1)
			: !1;
	}
	var o, f, u;
	n.fn.dotdotdot ||
		((n.fn.dotdotdot = function (r) {
			var u;
			if (0 == this.length) return (r && r.debug === !1) || y(!0, 'No element found for "' + this.selector + '".'), this;
			if (this.length > 1)
			...
			(u = this),
				...
				(u.bind_events = function () {
					return (
						u
							.bind('update.dot', function (i, r) {
								i.preventDefault(),
									i.stopPropagation(),
									(f.maxHeight = 'number' == typeof f.height ? f.height : v(u)),
									(f.maxHeight += f.tolerance),
									r !== void 0 &&
										(('string' == typeof r || r instanceof HTMLElement) && (r = n('<div />').append(r).contents()),
										r instanceof n && (p = r)),
									(h = u.wrapInner('<div class="dotdotdot" />').children()),
									h
										.empty()
										.append(p.clone(!0))
										.css({ height: 'auto', width: 'auto', border: 'none', padding: 0, margin: 0 });
								var c = !1,
									o = !1;
								return (
									s.afterElement && ((c = s.afterElement.clone(!0)), s.afterElement.remove()),
									t(h, f) && (o = 'children' == f.wrap ? l(h, f, c) : e(h, u, h, f, c)),
									h.replaceWith(h.contents()),
									(h = null),
									n.isFunction(f.callback) && f.callback.call(u[0], o, p),
									(s.isTruncated = o),
									o
								);
							})
							.bind('isTruncated.dot', function (n, t) {
								return (
									n.preventDefault(),
									n.stopPropagation(),
									'function' == typeof t && t.call(u[0], s.isTruncated),
									s.isTruncated
								);
							})
							.bind('originalContent.dot', function (n, t) {
								return n.preventDefault(), n.stopPropagation(), 'function' == typeof t && t.call(u[0], p), p;
							})
							.bind('destroy.dot', function (n) {
								n.preventDefault(),
									n.stopPropagation(),
									u.unwatch().unbind_events().empty().append(p).data('dotdotdot', !1);
							}),
						u
					);
				}),
				...
				(u.watch = function () {
					if ((u.unwatch(), 'window' == f.watch)) {
						var t = n(window),
							e = t.width(),
							r = t.height();
						t.bind('resize.dot' + s.dotId, function () {
							(e == t.width() && r == t.height() && f.windowResizeFix) ||
								((e = t.width()),
								(r = t.height()),
								c && clearInterval(c),
						...
						(w = i(u)),
							(c = setInterval(function () {
								var n = i(u);
								(w.width != n.width || w.height != n.height) && (u.trigger('update.dot'), (w = i(u)));
							}, 100));
					return u;
				}),
				(u.unwatch = function () {
					return n(window).unbind('resize.dot' + s.dotId), c && clearInterval(c), u;
				});
			var p = u.contents(),
				f = n.extend(!0, {}, n.fn.dotdotdot.defaults, r),
				s = {},
				w = {},
				c = null,
				h = null;
			return (
				(s.afterElement = a(f.after, u)),
				(s.isTruncated = !1),
				(s.dotId = o++),
				u.data('dotdotdot', !0).bind_events().trigger('update.dot'),
				f.watch && u.watch(),
				u
			);
		}),
		...
		(o = 1),
		(f = n.fn.html),
		(n.fn.html = function (n) {
			return n !== void 0
				? this.data('dotdotdot') && 'function' != typeof n
					? this.trigger('update', [n])
					: f.call(this, n)
				: f.call(this);
		}),
		(u = n.fn.text),
		(n.fn.text = function (t) {
			if (t !== void 0) {
...
*/ //For whatever reason this kills all functions that run after the page had finished loading. // GenericModal.js

typeof Roblox.GenericModal == 'undefined' &&
	(Roblox.GenericModal = (function () {
		function i(t, i, u, f, e) {
			n = f;
			var o = $('div.GenericModal').filter(':first');
			o.find('div.Title').text(t),
				i === null ? o.addClass('noImage') : (o.find('img.GenericModalImage').attr('src', i), o.removeClass('noImage')),
				o.find('div.Message').html(u),
				e && (o.removeClass('smallModal'), o.addClass('largeModal')),
				o.modal(r);
		}
		function t() {
			$.modal.close(), typeof n == 'function' && n();
		}
		var r = { overlayClose: !0, escClose: !0, opacity: 80, overlayCss: { backgroundColor: '#000' } },
			n;
		return (
			$(function () {
	...
	})()); // ItemPurchase.js // CommentsPane.js
			...
			$.ajax({
				...
				success: function (n) {
					typeof n.ID != 'undefined'
						? (parseInt($('#AjaxCommentsPaneData').attr('data-comments-floodcheck'), 10) > 0
								? $('.AjaxCommentsContainer textarea')
										.addClass('hint-text')
										.val(
								...
								: $('.AjaxCommentsContainer textarea')
										.addClass('hint-text')
										.val(Roblox.CommentsPane.Resources.defaultMessage),
		...
		(Roblox.GenericConfirmation = (function () {
			function s(n) {
				var c = {
					...
					s;
				(n = $.extend({}, c, n)),
					(i.overlayClose = n.dismissable),
					(i.escClose = n.dismissable),
					(o = $('[roblox-confirm-btn]')),
					o.html(n.acceptText + "<span class='btn-text'>" + n.acceptText + '</span>'),
					o.attr('class', 'btn-large ' + n.acceptColor),
					o.unbind(),
					o.bind('click', function () {
						return n.fieldValidationRequired ? f(n.onAccept) : t(n.onAccept), !1;
					}),
					(e = $('[roblox-decline-btn]')),
					e.html(n.declineText + "<span class='btn-text'>" + n.declineText + '</span>'),
					e.attr('class', 'btn-large ' + n.declineColor),
					e.unbind(),
					e.bind('click', function () {
						return t(n.onDecline), !1;
					}),
					$('[data-modal-handle="confirmation"] div.Title').text(n.titleText),
					(h = $("[data-modal-handle='confirmation']")),
					n.imageUrl == null
						? h.addClass('noImage')
						: (h.find('img.GenericModalImage').attr('src', n.imageUrl), h.removeClass('noImage')),
					n.allowHtmlContentInBody
						? $("[data-modal-handle='confirmation'] div.Message").html(n.bodyContent)
						: $("[data-modal-handle='confirmation'] div.Message").text(n.bodyContent),
					$.trim(n.footerText) == ''
					...
					n.allowHtmlContentInFooter
						? $('[data-modal-handle="confirmation"] div.ConfirmationModalFooter').html(n.footerText)
						: $('[data-modal-handle="confirmation"] div.ConfirmationModalFooter').text(n.footerText),
					$("[data-modal-handle='confirmation']").modal(i),
					(s = $('a.genericmodal-close')),
					s.unbind(),
			...
			var o = 'btn-primary',
				...
				r = 'btn-negative',
				e = 'btn-none',
				i = { overlayClose: !0, escClose: !0, opacity: 80, overlayCss: { backgroundColor: '#000' } };
			return { open: s, close: n, green: o, blue: u, gray: r, none: e };
		})()); // Item.js
...
$(document).ready(function () {
	...
	function r() {
		...
		t && (!i || n) && $('.ItemOptions, .ItemOptions .menu .invisible').removeClass('invisible'),
			n || $('.ItemOptions .delete-item-btn').removeClass('invisible');
	}
	var n = $('.DescriptionPanel .Description.Full').text(),
		t;
	n.length > 150 && (n = n.substring(0, 147) + '... <a onclick="Roblox.Item.toggleDesc(\'more\');">More</a>'),
		$('.DescriptionPanel .Description.body').html(n),
		$('.PutItemUpForSaleBtn').click(function (n) {
			n.preventDefault();
			var t = { escClose: !0, opacity: 80, overlayCss: { backgroundColor: '#000' }, appendTo: 'form' };
		...
		$('.SetAddButton').click(function (n) {
			...
			var t = $(this),
				...
				r = t.attr('setid'),
				i = 'waiting' + r + '_' + u;
			t.append("<img src='/images/spinners/spinner16x16.gif' id='" + i + "'"),
				$.ajax({
					type: 'POST',
					async: !0,
					cache: !1,
					...
					success: function (n) {
						n !== null &&
							(t.removeClass('SetAddButton'),
							t.addClass('SetAddButtonAlreadyContainsItem'),
							t.unbind('click'),
							$('#' + i).remove());
					},
					failure: function (n) {
	...
	$('.SetOptions .btn-dropdown').click(function () {
		return i('.SetOptions'), !1;
	}),
		$('.ItemOptions .btn-dropdown').click(function () {
			return i('.ItemOptions'), !1;
		}),
		...
		(Roblox.CatalogItemPurchase = new Roblox.ItemPurchase(function (n) {
			if (n.IsMultiPrivateSale)
				$('.ConfirmationModal').on('remove', function () {
		...
		Roblox.Item.ShowAssetGrantedModal &&
			((t = null),
			Roblox.Item.ForwardToUrl && (t = window.open(Roblox.Item.ForwardToUrl, '_blank')),
			Roblox.GenericModal.open(
				Roblox.Resources.AssetGrantedResources.assetGrantedModalTitle,
				null,
				Roblox.Resources.AssetGrantedResources.assetGrantedModalMessage,
				null,
			),
			Roblox.Item.ForwardToUrl &&
				...
				$('#simplemodal-container').bind('destroyed', function () {
					document.location.href = Roblox.Item.ForwardToUrl;
				})));
}),
	(Roblox.Item = (function () {
		...
		function f() {
			...
			var f = Math.round(t * n),
				i = f > 1 ? f : 1,
				r = t - i;
			$('#SellItemModalContainer .lblCommision').text(i > 0 ? i : ''),
				$('#SellItemModalContainer .lblLeftover').text(r >= 0 ? r : '');
		}
		function r(n) {
			var r = $('.DescriptionPanel .Description.body'),
				...
				i;
			n == 'more'
				? ((t = 'Less'), r.html(u.text() + '   <a onclick="Roblox.Item.toggleDesc(\'less\');">' + t + '</a>'))
				: n == 'less' &&
				  ((i = 'More'), r.html(u.text().substring(0, 147) + '... <a onclick="Roblox.Item.toggleDesc(\'more\');">' + i + '</a>'));
		}
		function t(n) {
			var t = { overlayClose: !0, opacity: 80, overlayCss: { backgroundColor: '#000' } };
			typeof n != 'undefined' && n !== '' && $.modal.close('.' + n), $('#ProcessingView').modal(t);
		}
		function i(n, t, i) {
			...
			$.post('/Sets/SetHandler.ashx?rqtype=getnewestversion&assetSetItemId=' + n + (t ? '&allsets=true' : ''), function () {
				$('.UpdateInSetsContainerDiv').remove(),
					$('a[setid=' + r + ']')
						.removeClass('SetAddButton')
						.addClass('SetAddButtonAlreadyContainsItem');
			});
		}
		var n = 0.3;
		return { showProcessingModal: t, updateSets: i, toggleDesc: r, validateResellInput: f, setMarketPlaceFee: u };
	})());
