...
Roblox.Popover = (function () {
	...
	function setUpTrianglePosition(popoverContentElm, popoverContainerElm) {
		...
		if (popoverContentSelector.hasClass('bottom') || popoverContentSelector.hasClass('top')) {
			var popoverContainerOffsetLeft =
				$('body').outerWidth() - parseInt(popoverContainerSelector.width() + popoverContainerSelector.offset().left);
			right =
				$('body').outerWidth() - popoverTriggerOffsetLeft - popoverContainerOffsetLeft - popoverTrigerWidth / 2 - triangleWidth / 2;
	...
	function closePopover() {
		$('body').on('click', function (e) {
			$(popoverClassName).each(function () {
				...
				if ($(popoverContentSelector).hasClass(popoverAlwaysOpenClassName) && !$(e.target).hasClass(popoverCloseClassName)) {
					return false;
				}
