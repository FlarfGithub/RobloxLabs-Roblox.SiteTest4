...
Roblox.AdsHelper.AdRefresher = (function () {
	...
	var maxWidthIncludeTwoAds =
		$('[data-max-width-for-two-ads]').attr('data-max-width-for-two-ads') ||
		defaultWidth + skyscraperWidth * 2 + 12 * 4 - browserScrollbarWidth;
	// include 970px default width + two side ads width + gaps(24px * 2)
	var maxWidthIncludeOneAd =
		$('[data-max-width-for-one-ads]').attr('data-max-width-for-one-ads') ||
		defaultWidth + skyscraperWidth + 12 * 2 - browserScrollbarWidth;
...
Roblox.AdsHelper.DynamicAdCreator = (function () {
	function populateNewAds() {
		...
		adsToPopulate.each(function (idx, el) {
			...
			googletag.cmd.push(function () {
				var slot = googletag.defineSlot(adSlot, [adWidth, adHeight], adId).addService(googletag.pubads());
