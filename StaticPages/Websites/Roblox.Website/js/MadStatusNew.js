MadStatus = {
	...
	newLib: function () {
		return (libString =
			this.participle[Math.floor(Math.random() * this.participle.length)] +
			' ' +
	...
	start: function () {
		if (MadStatus.timeout == null) MadStatus.timeout = setInterval('MadStatus.update()', MadStatus.updateInterval);
	},
	stop: function (msg) {
		if (MadStatus.running) {
			clearInterval(MadStatus.timeout);
			MadStatus.timeout = null;
			if (typeof msg != typeof undefined) MadStatus.field[0].innerHTML = msg;
			else MadStatus.field[0].innerHTML = '';
		}
	},
	manualUpdate: function (staticMsg, resumeAfterUpdate, animate) {
		if (MadStatus.timeout) MadStatus.stop();

		this.update(staticMsg, animate);

		if (resumeAfterUpdate) setTimeout('MadStatus.start()', 1000);
	},
	update: function (staticMsg, animate) {
		if (typeof staticMsg != typeof undefined) MadStatus.backBuffer[0].innerHTML = staticMsg;
		else MadStatus.backBuffer[0].innerHTML = this.newLib();

		if (typeof noAnim != typeof undefined && animate == false) return;
