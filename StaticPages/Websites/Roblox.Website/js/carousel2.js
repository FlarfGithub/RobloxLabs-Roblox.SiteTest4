	...
	for (i = 1; i <= 5; i++) {
		...
		if (i == 1) {
			eval(
				"$('.nav" +
					i +
					"').mouseover(function () { stopGlowButtons(); $('.tooltip').attr('style', 'position: absolute; top: 159px; left: " +
					(198 + 30 * i) +
					"px'); $('#game').attr('src', carouselArray[" +
					(i - 1) +
					'].img_thumb); });',
			);
		} else {
			eval(
				"$('.nav" +
					i +
					"').mouseover(function () { stopGlowButtons(); $('.tooltip').attr('style', 'position: relative; top: 159px; left: " +
					(198 + 30 * i) +
					"px'); $('#game').attr('src', carouselArray[" +
					(i - 1) +
					'].img_thumb); });',
