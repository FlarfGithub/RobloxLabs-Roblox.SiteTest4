...
Roblox.DeveloperStats = (function () {
	...
	$(function () {
					...
					if (isPromotion) {
						newRow = $(
							'<tr class="' +
								...
								timeStamp +
								'"><td class="first">' +
								Roblox.FlotGraphing.ConvertTimeToReadableString(timeStamp, visibleTime) +
								'</td><td class="promotion-acquisitions"></td><td class="promotion-conversions"></td><td class="promotion-revenue"></td></tr>',
						);
					} else if (isGame) {
						newRow = $(
							'<tr class="' +
								...
								'"><td class="first">' +
								Roblox.FlotGraphing.ConvertTimeToReadableString(timeStamp, visibleTime) +
								'</td><td class="game-traffic-visits"></td><td class="game-traffic-average-time"></td><td class="game-revenue-robux"></td></tr>',
						);
					}
			...
			chartContainers.each(function () {
				...
				$.ajax({
					...
					success: function (data, status, xhr) {
						if (xhr.status === 202) {
							//request queued, but we have no data, so don't bother charting it
							subCharts.each(function () {
								$(this).removeClass('loading').addClass('waiting');
								$(this).text(
									'This information is currently being calculated. The chart will automatically load when the calculation is finished.',
								);
							});
						} else {
		...
		if (Roblox.RealTime && Roblox.RealTime.Factory) {
			...
			realTimeClient.Subscribe('RevenueReports', function (data) {
				if (data.Type === 'DeveloperProductRevenueAggregationCalculated' && data.UniverseId === universeId) {
					var visibleTab = $('.content-container .tab-active');
					var visibleTime = $('.nav.nav-pills li.active', visibleTab).data('rbx-time');
					drawDelayedPieCharts(visibleTab, visibleTime);
				}
