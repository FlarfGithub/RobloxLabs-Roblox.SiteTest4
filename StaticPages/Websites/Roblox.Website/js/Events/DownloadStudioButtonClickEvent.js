...
Roblox.DownloadStudioButtonClickEvent = (function () {
	var sendEvents = function (pageName, suffix) {
		typeof GoogleAnalyticsEvents !== 'undefined' &&
			...
			GoogleAnalyticsEvents.FireEvent([pageName + suffix, 'DownloadStudioButtonClick']);
	...
	var init = function () {
		...
		$('.studio-launch-header').on('click', function () {
			sendEvents(pageName, '-header');
		});

		$('.studio-launch-image').on('click', function () {
			sendEvents(pageName, '-image');
		});

		$('.studio-launch-footer').on('click', function () {
			sendEvents(pageName, '-footer');
		});
	};
