...
var MyItemPage = new (function () {
	...
	function GetMarketplaceFee(currencyType) {
		...
		if (derivedFee > minimumFee) fee = derivedFee;
		else fee = minimumFee;
	...
	function CalculateFeeAndProfit(currencyType) {
		if (currencyType == 'Robux') {
			...
			if (MinPriceTickets != MaxPriceTickets) {
				...
				ValidatePricing(
					'.PricingField_Tickets [type="text"]',
					Math.max(MinMarketplaceFeeTickets, MinPriceTickets),
					MaxPriceTickets,
				);
			}
	...
	function ClearText() {
		if (!$('.PricingField_Robux [type=checkbox]').attr('checked')) {
			...
			if (!RobuxTextBoxDefault) RobuxTextBoxDefault = !valFieldRobux ? MinPriceRobux : valFieldRobux;
		...
		if (!$('.PricingField_Tickets [type=checkbox]').attr('checked')) {
			...
			if (!TicketsTextBoxDefault) TicketsTextBoxDefault = !valFieldTickets ? MinPriceTickets : valFieldTickets;
	...
	this.Initialize = function (
		minMarketplaceFeeRobux,
		minMarketplaceFeeTickets,
		marketplaceTaxRate,
		minPriceRobux,
		minPriceTickets,
		maxPriceRobux,
		maxPriceTickets,
		userHasBCSellRequirement,
		userIsSellerBanned,
		placeAssetType,
		priceChangeable,
	) {
		...
		$('.PersonalServerAccessCtrls input,select').change(function () {
			madeChanges = true;
		});
		...
		window.onbeforeunload = function () {
			if (madeChanges) return 'Your changes have not been saved.';
		};
