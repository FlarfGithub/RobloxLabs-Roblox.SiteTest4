	...
	function showInsufficientFundsModal(robuxRemaining, alerturl) {
		...
		Roblox.GenericConfirmation.open({
			...
			bodyContent: String.format(
				Roblox.ChangeUsername.Resources.insufficientFundsText,
				"<span class='robux notranslate'>" + addCommas(robuxRemaining) + '</span>',
			),
			acceptText: Roblox.ChangeUsername.Resources.insufficientFundsAcceptText,
			acceptColor: Roblox.GenericConfirmation.green,
			onAccept: function () {
				window.location.href = buyRobuxUrl;
			},
	...
	function initializeChangeUsernameButton() {
		$('#changeUsername').click(function () {
			...
			if (usernameSetting.data('email-verified') == 'False') {
				...
				var processingImage =
					"<div style='clear:both;padding-bottom:10px;'></div><div style='height:10px;padding-left:45px;'><div id='Processing' style='display:none;'><img src='" +
					processIndicatorImageUrl +
					"' alt='" +
					processingText +
					"'/></div></div>";
				...
				Roblox.GenericConfirmation.open({
					...
					titleText: Roblox.ChangeUsername.Resources.changeUsernameTitle,
					bodyContent:
						inputFieldsWrapperStart + errorFields + newUsernameField + passwordField + inputFieldsWrapperEnd + processingImage,
...
$(function () {
	...
	function checkIfUserUnder13() {
		...
		var under13Date = new Date(
			$('#AccountPageContainer').data('currentdateyear'),
			$('#AccountPageContainer').data('currentdatemonth'),
			$('#AccountPageContainer').data('currentdateday'),
		);
	...
	$('.updateSettingsBtn').on('click', function (evt) {
		...
		if (!$('.updateSettingsBtn').hasClass('btn-disabled-neutral')) {
			if (userAbove13 && checkIfUserUnder13() && !RobloxMissingParentEmail.hasParentEmailBeenReset) return false;
		} else {
			return false;
		}
...
$(function () {
	...
	$('#AskParentToVerifyAgeLink').on('click', function (evt) {
		...
		$.ajax({
			...
			type: 'post',
			success: function (status) {
				Roblox.GenericModal.open('Age Verification', null, status, function () {}, false);
			},
		});
	});
	...
	$('#AskParentToAgeMeUp').on('click', function (evt) {
		...
		$.ajax({
			...
			type: 'post',
			success: function (status) {
				Roblox.GenericModal.open('Age Up', null, status, function () {}, false);
			},
		});
	});
