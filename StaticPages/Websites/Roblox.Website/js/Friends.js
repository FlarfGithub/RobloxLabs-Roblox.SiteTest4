...
var InitializeFriends = function (n) {
	var t = function (u) {
			...
			Roblox.require('Widgets.AvatarImage', function (n) {
				...
				(u == 'Friends' || u == 'BestFriends') &&
					(u == 'Friends' &&
						((s = $('#' + u + 'Tab .add-best-friend')),
						s.click(function () {
							i('/friends/addbestfriend', this);
						})),
					(h = $('#' + u + 'Tab .remove-best-friend')),
					h.click(function () {
						i('/friends/removebestfriend', this);
					}),
					(o = $('#' + u + 'Tab .remove-friend')),
					o.click(function () {
						var n = $(this);
				...
				u == 'FriendRequests' &&
						...
						if (i.attr('disabled') !== undefined)
							return (
								Roblox.GenericModal.open(
									Roblox.Friends.Resources.FeatureDisabled,
									null,
									Roblox.Friends.Resources.AddFriendsDisabled,
								),
								!1
							);
						(n = {
					...
					(f = $('#' + u + 'Tab .accept-friend')),
					f.click(function () {
						i('/friends/acceptfriendrequest', this);
					}),
					(e = $('#' + u + 'Tab .decline-friend')),
					e.click(function () {
				...
				n
					? $('#' + u + 'Tab .friend-hover .friend-dropdown').show()
					: $('#' + u + 'Tab .friend-hover').hover(
							function () {
		...
		i = function (n, i) {
			...
			$('#' + r + 'Tab').load(n, u, function () {
				t(r),
					r == 'Friends' &&
						$('#BestFriendsTab').load(
							'/friends/bestfriends?DisplayedUserID=' + $(i).data('displayed-user-id') + '&PageNum=1',
							function () {
								t('BestFriends');
							},
						),
					(r == 'BestFriends' || r == 'FriendRequests') &&
						$('#FriendsTab').load(
							'/friends/friends?DisplayedUserID=' + $(i).data('displayed-user-id') + '&PageNum=1',
							function () {
								t('Friends');
							},
						);
