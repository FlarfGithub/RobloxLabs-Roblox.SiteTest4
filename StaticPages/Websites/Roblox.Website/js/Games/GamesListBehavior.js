...
(Roblox.GamesListBehavior = {}),
	...
	(Roblox.GamesListBehavior.NumberOfGamesToAppendInHScrollMode = 20);
	...
	(Roblox.GamesListBehavior.GamesListObject.prototype = {
		populateGamesList: function (n) {
			...
			t.jqxhr = $.get(u, r, function (n) {
				...
				(t.jqxhr = null),
					(u = $('<div></div>').append(n)),
					Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode() || t.hideExtraGames(u),
					(i = $('#' + t.divId + ' .games-list')),
					$(i).find('.game-item,.games-list-column').remove(),
					(i = Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode() ? $('#' + t.divId + ' .horizontally-scrollable') : i),
					i.append(u.html()),
					(t.numberOfGamesOnScreen = $(i).find('.game-item').length),
					t.hideLoadingIndicator(),
					Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode() &&
						((r = $('#' + t.divId + ' .horizontally-scrollable')),
						r.length > 0 && ($(r).css('left', 0), $(r).siblings('.scroller.prev').addClass('hidden')),
						(t.noMoreGamesToLoad = t.numberOfGamesOnScreen < t.numberOfGamesToFetch));
			});
		},
		appendToGamesList: function (n) {
			var t, u, f, i, r;
			this.jqxhr ||
				(Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode() && this.reachedHorizontalScrollMax) ||
				((t = this),
				(u = Roblox.GamesPageContainerBehavior.getURLBasedOnSortFilter(t.sortFilter)),
				(n = Math.min(n, Roblox.GamesListBehavior.MaxNumberOfGamesToLoadPerRequest)),
				(f = Roblox.GamesPageContainerBehavior.isTopRatedDefaultToWeeklyEnabled()),
				(i = Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode()),
				f && i && t.sortFilter == 11 && (t.timeFilter = 2),
				t.unhideGames(),
				(r = {
				...
				t.showLoadingIndicator(),
				(t.jqxhr = $.get(u, r, function (i) {
					var u, r;
					(t.jqxhr = null),
						(u = $('<div></div>').append(i)),
						Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode() || t.hideExtraGames(u),
						(r = Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode()
							? $('#' + t.divId + ' .horizontally-scrollable')
							: $('#' + t.divId + ' .games-list')),
						r.append(u.html()),
						(t.numberOfGamesOnScreen = $(r).find('.game-item').length),
						t.hideLoadingIndicator(),
						Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode() &&
							((t.noMoreGamesToLoad = $(i).find('.game-item').length < n),
							(t.reachedHorizontalScrollMax =
								t.numberOfGamesOnScreen > Roblox.GamesPageContainerBehavior.MaxNumberOfGamesToFetchInHScrollMode));
				})));
		},
		show: function (n) {
			n &&
				$('#' + this.divId + ' .games-list')
					.find('.game-item,.games-list-column')
					.remove(),
				(this.isShown = !0),
		...
		updateHeight: function () {
			$('#' + this.divId).height(this.maxHeight()),
				Roblox.GamesPageContainerBehavior.isInHorizontalScrollMode() &&
					$('#' + this.divId.games + ' .horizontally-scrollable').height(
						this.numberOfRowsToOccupy * Roblox.GamesPageContainerBehavior.RowHeightIncludingPadding,
					);
		},
		...
		handleHorizontalScroll: function (n) {
			...
			i.length > 0 &&
				($(i).stop('', !0, !0),
				(r = parseInt($(i).css('left')) || 0),
				(r = this.getClosestColumnBoundary(r)),
				n === 'prev'
					? ((t = r + this.getLeftBoundaryOfLastVisibleColumn()),
					  (t = Math.min(t, 0)),
					...
					: ((t = r - this.getLeftBoundaryOfLastVisibleColumn()),
					  ...
					  (this.noMoreGamesToLoad || this.reachedHorizontalScrollMax) &&
				...
				(u = $('#GamesRightColumn')[0]),
				u != null && u.contentWindow.Roblox.RightColumnBehavior.refreshAds(),
				(f = t < 0),
				f ? $(i).siblings('.scroller.prev').removeClass('hidden') : $(i).siblings('.scroller.prev').addClass('hidden'));
		},
		...
		maxHeight: function () {
			...
			return (
				this.isShown &&
					((n += this.numberOfRowsToOccupy * Roblox.GamesPageContainerBehavior.RowHeightIncludingPadding),
					(n += Roblox.GamesPageContainerBehavior.GamesListHeaderHeight),
		...
		unhideGames: function () {
			...
			(this.numberOfGamesOnLastRow = n % Roblox.GamesPageContainerBehavior.getNumberOfColumns()),
