...
Roblox.GamesPageContainerBehavior = (function () {
	...
	function wi() {
		for (
			var u = -1,
				e = parseInt($('#GamesListsContainer').css('min-height')) || 0,
				o = Math.max($(window).height(), e) - (di + (it + nr) * c),
				f = Math.floor(o / ht),
				r = t ? 1 : Math.max(1, Math.floor((2 * f) / c)),
				i = 0;
			i < n.length;
			i++
		)
			n[i].isShown && ((n[i].numberOfRowsToOccupy = r), (u = i));
	}
	...
	function cr() {
		for (var r = !0, u = 0, i, t = 0; t < n.length; t++)
			n[t].isShown && (r ? (r = !1) : ((i = (u++ + 1) * ir), $('#' + n[t].divId).css('top', '-' + i + 'px')));
	...
	function vr(n) {
		$.inArray(n, yt) !== -1 ? $('#TimeFilter').attr('disabled', 'disabled') : $('#TimeFilter').removeAttr('disabled'),
			$.inArray(n, lt) !== -1 ? $('#GenreFilter').attr('disabled', 'disabled') : $('#GenreFilter').removeAttr('disabled');
	}
	...
	function hr(n) {
		ii(),
			...
			$('#GamesPageRightColumnSidebar').length > 0 &&
				(n !== 'default' ||
					$('#GamesPageRightColumnSidebar').hasClass(s) ||
					($('#GamesPageRightColumnSidebar').removeClass(), $('#GamesPageRightColumnSidebar').addClass(s)));
	}
	...
	function or() {
		$('#SortFilter').val($('#SortFilter').data('default')),
			...
			$('#SiteFilter').val($('#SiteFilter').data('default'));
	}
			...
			($('div.filter select').each(function () {
				(filterType = $(this).attr('id')),
					...
					selectedVal &&
						filterType &&
						!$(this).attr('disabled') &&
						((r += (r === '' ? '' : '&') + filterType + '=' + selectedVal), (u[filterType] = selectedVal));
			}),
			!t)
		)
	...
	function ti() {
		$(document).on('GuttersHidden', function () {
			h &&
				...
				(r = $('#RightGutterAdContainer')),
				$('#GamesPageLeftColumn').css('margin', '0 345px 0 10px'),
				i.hide(),
				r.hide(),
				$('#GutterAdStyles').remove(),
				$('#GamesPageRightColumnSidebar').html(
					"<iframe id='GamesRightColumn' src='/games/rightcolumn' scrolling='no' frameBorder='0' style='height:550px;width:330px;border:0px;overflow:hidden'></iframe>",
				));
		});
	}
	...
	function et() {
		...
		h &&
			...
			(t = $('body').width()),
			t <= k
				? ($('#GamesPageLeftColumn').css('margin', '0'), i.hide(), r.hide())
				: ((n = gt(t)),
				  i.css('left', n - l + 'px'),
				  r.css('right', n - l + 'px'),
				  i.show(),
				  r.show(),
				  $('#GamesPageLeftColumn').css('margin', '0 ' + (n + 10) + 'px 0 ' + (n + 10) + 'px')));
	}
	...
	function st(t) {
		for (var i = 0; i < n.length; i++) if (n[i].divId === 'GamesListContainer' + t) return n[i];
		return null;
	}
	...
	function si(n) {
		if (Roblox.GamesPageContainerBehavior.FilterValueToGamesListsIdSuffixMapping.hasOwnProperty(n)) {
			t = !0;
			for (var i = 0; i < Roblox.GamesPageContainerBehavior.FilterValueToGamesListsIdSuffixMapping[n].length; i++)
				rt(Roblox.GamesPageContainerBehavior.FilterValueToGamesListsIdSuffixMapping[n][i], t);
	...
	function ci() {
		(f = $(window).width()),
			(o = $(window).height()),
			$('#GamesPageRightColumnSidebar').length > 0 && (at = $('#GamesPageRightColumnSidebar').position().top);
	}
	...
	return (
		$(function () {
			(document.title = Roblox.GamesPageContainerBehavior.Resources.pageTitle),
				$('#SortFilter [data-hidetimefilter]').each(function (n, t) {
					yt.push($(t).val());
				}),
				$('#SortFilter [data-hidegenrefilter]').each(function (n, t) {
					lt.push($(t).val());
				}),
				(nt = $('#FiltersAndSort').data('defaultweeklyratings')),
				pi(),
				$('#GamesPageRightColumnSidebar').length > 0 && (s = $('#GamesPageRightColumnSidebar').attr('class')),
				ar(),
				...
				(r = $('#RightGutterAdContainer')),
				(h = i.length !== 0),
				(wt = $('#TopAdContainer').length !== 0),
				$('.games-filter-resetter').click(ni),
				$('.games-filter-changer').click(bt),
				h
				...
				wt && Roblox.AdsHelper.AdRefresher.registerAd('TopAdContainer'),
				ct || ut(e.getState().data),
				v();
			...
			i > f || r > o
				? setTimeout(function () {
						if ((v(), !t)) {
							...
							i >= 980 &&
								$('#GamesPageRightColumnSidebar.move-with-window-min-left').toggleClass(
				  ...
				  setTimeout(function () {
						i < f && et(),
							t
								? v()
								: i < 980 &&
								  $('#GamesPageRightColumnSidebar.move-with-window-stick-right').toggleClass(
										'move-with-window-min-left move-with-window-stick-right',
								  ),
							(f = i),
							(o = r);
		...
		$(window).scroll(function () {
			...
			t ||
				((n = li()),
				i + 60 > at
					? $('#GamesPageRightColumnSidebar').hasClass(n) ||
					  ($('#GamesPageRightColumnSidebar').removeClass(), $('#GamesPageRightColumnSidebar').addClass(n))
					: $('#GamesPageRightColumnSidebar').hasClass(s) ||
					  ($('#GamesPageRightColumnSidebar').removeClass(), $('#GamesPageRightColumnSidebar').addClass(s)),
				i > vt &&
					!t &&
					$(window).scrollTop() >= $(document).height() - $(window).height() - tt &&
					(kt(), fi(ot(), Roblox.GamesPageContainerBehavior.adRefreshRateMilliSeconds))),
				(vt = i);
		}),
		(b = null),
		{
