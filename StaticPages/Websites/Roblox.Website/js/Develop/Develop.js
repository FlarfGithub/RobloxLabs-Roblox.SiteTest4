...
Roblox.DevelopPage = (function () {
	var ignoreUrlChange = false,
		...
		isFirstTimeLoadingLibrary = false;
	...
	$(function () {
		...
		if (document.URL.indexOf(libraryTabLinkUrl) !== -1) {
			...
			if (
				libraryTabQueryParams &&
				Roblox.CatalogValues &&
				Roblox.CatalogValues.CatalogContentsUrl &&
				hasCatalogContextParam &&
				hasCategoryParam
			) {
				Roblox.CatalogShared.LoadCatalogAjax(
					Roblox.CatalogValues.CatalogContentsUrl + '?' + libraryTabQueryParams,
					null,
					$libraryTabContentElement,
					true,
				);
			}
			// ... or the defaults for the library.
				...
				Roblox.CatalogShared.LoadCatalogAjax(
					$libraryTabLinkElement.data('library-get-url'),
					null,
					$libraryTabContentElement,
					false,
					true,
		...
		$libraryTabContentElement.on(
			Roblox.CatalogShared.CatalogLoadedViaAjaxEventName,
			null,
			null,
			Roblox.CatalogShared.handleCatalogLoadedViaAjaxEvent,
		);
		...
		$('.develop-experimental-label').click(function () {
			...
			Roblox.Dialog.open({
				...
				bodyContent:
					"Experimental mode games are games made by new developers. These games may not be stable and unexpected things could happen here.<br/><a class='text-link' target='_blank' href='" +
					wikiUrl +
					"'>Learn more from Wiki</a>",
				showAccept: false,
				declineText: 'OK',
	...
	function handleURLChange(state) {
		if (!ignoreUrlChange && state.clickTargetID) {
			...
			if (state.clickTargetID === 'catalog' || state.clickTargetID === libraryTabLinkID) {
				//&& state.url
	...
	function handleTabChange($targetTabLink) {
		...
		if (!doNotUpdateHistory) {
			// If we're here because of a history change, it means history already knows about this transition.
			ignoreUrlChange = true;
