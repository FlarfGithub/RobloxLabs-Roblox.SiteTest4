$(function () {
	...
	var invalidFileExtension = isSound
		? Roblox.EmbeddedUpload.Resources.invalidSoundFile
		: Roblox.EmbeddedUpload.Resources.invalidImageFile;
	var validFileExtensions = isSound ? ['.mp3'] : ['.png', '.jpg', '.jpeg', '.jpe', '.bmp'];
	var postMessageMethods = {
		verifyCaptcha: 'verifyCaptcha',
	};

	if (isSound && $('#isOggUploadEnabled').val().toLowerCase() === 'true') validFileExtensions.push('.ogg');
	if (!isSound && $('#isTgaUploadEnabled').val().toLowerCase() === 'true') validFileExtensions.push('.tga');
	...
	$('#file').change(function () {
		...
		if (!$('#file').val()) return; // no file selected
	...
	function verifyCaptcha(cb) {
		...
		window.addEventListener('message', function (event) {
			if (event.origin === window.location.origin) {
				if (event.data && event.data.method === postMessageMethods.verifyCaptcha) {
