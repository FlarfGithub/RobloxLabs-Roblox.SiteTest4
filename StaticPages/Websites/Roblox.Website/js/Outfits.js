typeof Roblox == 'undefined' && (Roblox = {}),
	(Roblox.InitializeOutfits = (function () {
		function t() {
			function a(t) {
				var i = String.format(
						"<p>{0}</p><table style='margin: auto'><tbody><tr><td><label class='form-label'>{1}</label></td><td><input class='outfitNameTextBox text-box text-box-large'></td></tr></tbody></table>",
						Roblox.Outfits.Resources.createText,
						Roblox.Outfits.Resources.outfitNameTextBoxLabel,
					),
					r = function () {
			...
			Roblox.require('Widgets.AvatarImage', function (n) {
				...
				(e = $('#OutfitsTab .outfit-avatar,#OutfitsTab .wear-outfit')),
				e.click(function () {
					n('/outfits/wear', this, function () {
						refreshAllUpdatePanels();
					});
				}),
				(s = $('#OutfitsTab .download-outfit')),
				s.click(function () {
					r($(this).data('target-useroutfit-id'));
				}),
				(h = $('#OutfitsTab .delete-outfit')),
				h.click(function () {
					var t = $(this),
				...
				(c = $('#OutfitsTab .rename-outfit')),
				c.click(function () {
					o($(this));
				}),
				(l = $('#OutfitsTab .update-outfit')),
				l.click(function () {
					var t = $(this),
						i = $(t).parentsUntil('.outfit-container').find('.roblox-avatar-image img').attr('src');
					Roblox.GenericConfirmation.open({
				...
				$('#OutfitsTab .outfit-hover .outfit-dropdown').show(),
				$('#OutfitsTab .button.gear').click(function () {
					$(this).hasClass('active') ||
						$('#OutfitsTab .button.gear.active').each(function () {
		...
		function o(n) {
			var t = String.format(
					"<p>{0}</p><table style='margin: auto'><tbody><tr><td><label class='form-label'>{1}</label></td><td><input class='outfitNameTextBox text-box text-box-large'></td></tr></tbody></table>",
					Roblox.Outfits.Resources.renameText,
					Roblox.Outfits.Resources.outfitNameTextBoxLabel,
				),
				i = function () {
		...
		function n(n, i, r) {
			var o = $(i).data('view'),
				u;
			$(i).data('target-useroutfit-id') && h(i),
				(u = e({
		...
		function r(n) {
			...
			$('#ProcessingView').modal(t),
				$.ajax('/outfit-thumbnail/json', {
					...
					success: function (t) {
						t.Final
							? ($.modal.close(), (window.location = '/outfits/download?userOutfitId=' + n))
							: setTimeout(function () {
									r(n);
							  }, 1e3);
					},
		...
		var u = !1;
		$(function () {
			(u = $('#OutfitsTab').data('isiosapp') == 'true'),
				t('Outfits'),
