...
Roblox.JSErrorTracker = {
	...
	processException: function (exDetails, pixelToFire) {
		...
		if (typeof exDetails.category === 'undefined') exDetails.category = Roblox.JSErrorTracker.data.category;
		switch (pixelToFire) {
			case 'GA':
	...
	createURL: function (url, args, mapping) {
		...
		if (distilled != null) {
			for (var arg in distilled) {
				if (typeof arg != typeof undefined && args.hasOwnProperty(arg)) urlToFire += arg + '=' + distilled[arg] + '&';
			}
		}
