...
(function () {
	...
	jQuery.fn = jQuery.prototype = {
		...
		add: function (selector) {
			return this.pushStack(
				jQuery.unique(jQuery.merge(this.get(), typeof selector === 'string' ? jQuery(selector) : jQuery.makeArray(selector))),
			);
		},
					...
					jQuery.each(types.split(/\s+/), function (index, type) {
						...
						if (events[type]) {
							...
							// remove all handlers for the given type
								...
								for (var handle in events[type]) if (namespace.test(events[type][handle].type)) delete events[type][handle];
	...
	jQuery.fx.prototype = {
		...
		step: function (gotoEnd) {
			...
			if (gotoEnd || t >= this.options.duration + this.startTime) {
				...
				this.pos = jQuery.easing[this.options.easing || (jQuery.easing.swing ? 'swing' : 'linear')](
					this.state,
					n,
					0,
					1,
