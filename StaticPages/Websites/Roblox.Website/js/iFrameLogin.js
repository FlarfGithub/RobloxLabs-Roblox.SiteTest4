...
Roblox.iFrameLogin = new (function () {
	...
	function init() {
		...
		var validateInputs = function () {
			...
			if (captchaOn) inputs.push($('#recaptcha_response_field'));
			jQuery.each(inputs, function () {
				var $input = $(this);
				if ($input.val() == '') {
					displayInputError($input, true);
					invalid = true;
				} else displayInputError($input, false);
			});
			return invalid;
		};
		...
		var displayInputError = function (div, mode) {
			...
			showLoggingIn(false);
			if (mode) div.css({ 'background-color': '#FDD' });
			else div.css({ 'background-color': 'white' });
		};
		...
		var submitLogin = function () {
			...
			if (captchaOn) $('#Captcha_upBadCaptcha').text('');
			var onSuccess = (onError = function (result, context) {
				if (result.IsValid) {
					...
					if (topUrl.indexOf('#') != -1) topUrl = window.parent.location.href.split('#')[0];
					if (topUrl.indexOf('?') == -1) topUrl += '?nl=true';
					else topUrl += '&nl=true';
					...
					//Else all other errors are to do with Captcha -if(result.Message == 'incorrect-captcha-sol')
						...
						else $('#Captcha_upBadCaptcha').text(result.Message);
					}
					if (captchaOn) {
