...
MongoListener.distillData = function (data, mapping) {
	...
	for (dataKey in mapping) {
		if (typeof data[dataKey] != typeof undefined) distilled[mapping[dataKey]] = encodeURIComponent(data[dataKey]);
	}

	return distilled;
};
MongoListener.fireEvent = function (shard, data) {
	$.ajax({ url: MongoListener.loggingURI + shard, dataType: 'jsonp', jsonpCallback: 'MongoListener.callback', data: data });
};
