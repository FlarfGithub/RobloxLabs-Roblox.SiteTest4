...
Roblox.CarouselDataPager = function (rootElem, settingsData, callbackFunction) {
	...
	var changeIndex = function (newIndex) {
		if (newIndex <= 0) {
			//equals sign so that we disable the arrow
			newIndex = 0;
		...
		if (totalNumOfObjects > settings.windowDisplay && (newIndex < currentIndex || newIndex == 0)) {
			//have enough items to page, and going up or at first
	...
	var contentIn = $('<div class="content-inner"></div>')
		.css({
			position: 'absolute',
			top: '0px',
			left: '0px',
		...
		.html(root.html());
	var contentOut = $('<div class="content-outer"></div>')
		.css({
			overflow: 'hidden',
