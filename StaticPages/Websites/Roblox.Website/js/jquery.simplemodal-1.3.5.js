	...
	$.modal.impl = {
		...
		create: function (data) {
			...
			s.d.container = $('<div></div>')
				...
				.css(
					$.extend({ position: s.o.fixed ? 'fixed' : 'absolute' }, s.o.containerCss, { display: 'none', zIndex: s.o.zIndex + 2 }),
				)
		...
		fixIE: function () {
			...
			$.each(
				[s.d.iframe || null, !s.o.modal ? null : s.d.overlay, s.d.container.css('position') === 'fixed' ? s.d.container : null],
				function (i, el) {
					if (el) {
		...
		getVal: function (v, d) {
			return v
				? typeof v === 'number'
					...
					? 0
					: v.indexOf('%') > 0
					? (parseInt(v.replace(/%/, '')) / 100) * (d === 'h' ? w[0] : w[1])
		...
		setContainerDimensions: function () {
			...
			var ch = s.d.origHeight
					...
					: s.getVal(badIE ? s.d.container[0].currentStyle['height'] : s.d.container.css('height'), 'h'),
				cw = s.d.origWidth
					? s.d.origWidth
					: browser.opera
					? s.d.container.width()
					: s.getVal(badIE ? s.d.container[0].currentStyle['width'] : s.d.container.css('width'), 'w'),
		...
		watchTab: function (e) {
			...
			if ($(e.target).parents('.simplemodal-container').length > 0) {
				...
				if (
					(!e.shiftKey && e.target === s.inputs[s.inputs.length - 1]) ||
