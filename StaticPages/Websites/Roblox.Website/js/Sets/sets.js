...
function LoadSets() {
	...
	if (currentSetView == SubscribedSetView) setRetrievalType = 'getsubscribedsets';

	$.getJSON('/Sets/SetHandler.ashx?rqtype=' + setRetrievalType + '&maxsets=10', function (data) {
		$('#LoadingSets').hide();
		...
		if (data.length < 1) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (i < 1) {
					innerHtml +=
						'<div id="setTabsListli' +
						id +
						'" alt="' +
						name +
						'" class="SetList-Set selected" onclick="CurrentlySorting(PopulateSet,' +
						id +
						');">';
				} else {
					innerHtml +=
						...
						'" alt="' +
						name +
						'" class="SetList-Set" onclick="CurrentlySorting(PopulateSet,' +
						id +
						');">';
				}
				innerHtml +=
					'<div class="SetList-SetContainer">' +
					'<div class="image"><img id="img_' +
					AssetSet.ImageAssetID +
					'" onclick="CurrentlySorting(PopulateSet,' +
					id +
					');" /></div>' +
					'<div class="name"><a id="setTab_' +
					id +
					'" onclick="CurrentlySorting(PopulateSet,' +
					id +
					');" >' +
				...
				innerHtml +=
					...
					id +
					'" style="display:none;">' +
					AssetSet.Description.escapeHTML() +
					'</div></div>';
...
function LoadAssetSetItems(setId) {
	...
	$.getJSON(
		'/Sets/SetHandler.ashx?rqtype=getsetitems&startRowIndex=' +
			currentPage[currentSetView][index] * numItemsShownPerPage +
			'&maximumRows=' +
			numItemsShownPerPage +
			'&setid=' +
			setId,
		function (objects) {
			if (objects !== null) {
				if (objects.length > 0 && objects[0].error) {
					...
					if (objects.length < 1) {
						...
						for (var i = 0; i < objects.length; i++) {
							...
							imagesHtml += GenerateAssetSetItemHtml(
								assetVersionId,
								itemname,
								assetSetItemID,
								isNewerVersionAvailable,
								AssetSetItem.SortOrder,
								false,
								AssetSetItem.AssetID,
							);
						}
...
function LoadAssetSetAllItems(setId) {
	...
	$.getJSON(
		'/Sets/SetHandler.ashx?rqtype=getsetitems&startRowIndex=0&maximumRows=' + totalNumberOfItemsInCurrentSet + '&setid=' + setId,
		function (objects) {
			if (objects !== null) {
				var imagesHtml = '';
				$('#AssetSetItems').empty();
				if (objects.length < 1) {
					...
					$('#AssetSetItems .item img').mousedown(function (event) {
...
function LoadSetInfo(setId) {
	...
	$.getJSON('/Sets/SetHandler.ashx?rqtype=getsetinfo&setid=' + setId, function (object) {
...
function GetThumbnail(assetVersionId, divId, attemptCount) {
	$.get('/Thumbs/RawAsset.ashx?AssetVersionID=' + assetVersionId + '&Width=110&Height=110&ImageFormat=png', function (data) {
		if (data !== null) {
			if (data == 'PENDING') {
				if (attemptCount < 4) {
					window.setTimeout(function () {
						GetThumbnail(assetVersionId, divId, attemptCount + 1);
					}, 3000);
				}
				return;
			}
...
function GetSetAssetImageThumbnail(assetId, elementID) {
	...
	$.get('/Thumbs/RawAsset.ashx?AssetID=' + assetId + '&Width=' + wh + '&Height=' + wh + '&ImageFormat=png', function (data) {
		if (data !== null) {
			if (data == 'PENDING') {
				$('#' + elementID).attr('src', '/images/spinners/spinner16x16.gif');
				window.setTimeout(function () {
					GetSetAssetImageThumbnail(assetId, elementID);
				}, 2000);
				return;
			}
...
function RemoveSetItem(assetSetItemId, divId) {
	...
	$.post('/Sets/SetHandler.ashx?rqtype=removefromset&assetSetItemId=' + assetSetItemId, function (data) {
		if (data !== null) {
...
function GenerateAssetSetItemHtml(assetVersionId, itemname, assetSetItemId, isNewerVersionAvailable, sortOrder, nolinks, assetId) {
	...
	html +=
		"<div class='item' id='" + divId + "' sortorder='" + sortOrder + "' assetId='" + assetId + "' assetVId='" + assetVersionId + "'>";
	if (nolinks) {
	...
	if (currentSetView == MySetView) {
		...
		html +=
			"<div id='removeButton_" +
			assetSetItemId +
			"' class='removeButton NewVersionDropDownItem' onclick=\"RemoveSetItemModal('" +
			assetSetItemId +
			"','" +
			divId +
			'\');" >Delete</div>';
		...
		if (isNewerVersionAvailable) {
			html +=
				"<div class='NewVersionDropDownItem' onclick=\"window.location='/item.aspx?setItemId=" +
				assetSetItemId +
				'&avID=' +
				assetVersionId +
				'\'">Preview Update</div>';
...
function GetNewVersion(assetSetItemId) {
	...
	$.post('/Sets/SetHandler.ashx?rqtype=getnewestversion&assetSetItemId=' + assetSetItemId, function (data) {
...
function DeleteSet(setId) {
	...
	$.post('/Sets/SetHandler.ashx?rqtype=deleteset&setId=' + setId, function (data) {
		$('#createSetTab').show();
		PopulateSetAfterDeletionOrUnSubscribing(setId);
	});
}
...
function UnsubscribeSet(setId) {
	...
	$.post('/Sets/SetHandler.ashx?rqtype=unsubscribe&setId=' + setId, function (data) {
		PopulateSetAfterDeletionOrUnSubscribing(setId);
	});
}
...
function ChangeSetView(newSetView) {
	...
	if (newSetView == MySetView) {
		...
		$('[roblox-create-find]')
			.attr('class', 'btn-large btn-neutral')
			.html(Roblox.Sets.Resources.findSet + "<span class='btn-text'>" + Roblox.Sets.Resources.findSet + '</span>');
		$('[roblox-create-find]').unbind('click');
		$('[roblox-create-find]').click(function () {
...
function UpdateSetHandler() {
	...
	UpdateDescription();
	if (!UpdateImage()) {
		//we need to chain updateimage and savesort, as we need them both to finish before we toggleEdit
	...
	$.ajax({
		...
		complete: function (xmlHttp) {
			if (xmlHttp.status != 200) {
				...
				$('#setTabsListli' + setId + ' .name').html(
					fitStringToWidthSafe(name.escapeHTML(), $($('.SetList-SetContainer .name')[0]).width()),
				);
...
function MoreDescription() {
	$('#SelectedSetDescription').html(
		$('#setdescription' + currentSetId[currentSetView]).html() +
			'<a id="MoreDescButton" onclick="LessDescription();return false;">&nbsp;' +
			Roblox.Sets.Resources.less +
			'</a>',
	);
}
...
function ShortenDescription(desc) {
	if (desc != null && desc.length > 300) {
		return (
			...
			'... <a id="MoreDescButton" onclick="MoreDescription();return false;">&nbsp;' +
			Roblox.Sets.Resources.more +
			'</a>'
		);
	} else {
		return desc;
...
function openMoveItemModal(elem) {
	...
	if ($('.moveItemContainerDiv .dropdown option:nth-child(1)').attr('value') != currentSetId[currentSetView]) {
		$('.moveItemContainerDiv .dropdown option[value="' + currentSetId[currentSetView] + '"]').insertBefore(
			$('.moveItemContainerDiv .dropdown option:nth-child(1)'),
		);
...
function SubmitMoveItem() {
	...
	$.ajax({
		...
		timeout: 50000,
		url:
			'/Sets/SetHandler.ashx?rqtype=addtoset&assetId=' +
			$('#div_setitem_' + setitemid).attr('assetid') +
			'&setId=' +
			$('.moveItemContainerDiv option:selected').val() +
			'&assetVId=' +
			$('#div_setitem_' + setitemid).attr('assetvid'),
		data: {},
		complete: function (xmlHttp) {
			if (xmlHttp.status != 200) {
				...
				RemoveSetItem(
					$('.moveItemContainerDiv #assetItemIDHidden').val(),
					'div_setitem_' + $('.moveItemContainerDiv #assetItemIDHidden').val(),
				);
				$.modal.close();
			}
		},
		...
		function () {
			...
			$(this)
				...
				.animate(
					...
					500,
					function () {
						$(this).parent('.outer').css('display', 'none').parent('.setitem_options').removeClass('true2');
					},
				);
		},
	);
