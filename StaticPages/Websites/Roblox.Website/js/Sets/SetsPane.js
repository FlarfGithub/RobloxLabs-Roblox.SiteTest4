...
Roblox.SetsPane = function (BaseUrl, DisplayedUserID) {
	...
	this.isVisible = function (elem) {
		if (
			elem.attr('style').toLowerCase().indexOf('display:none') > -1 ||
			elem.attr('style').toLowerCase().indexOf('display: none') > -1
		) {
			return false;
		}
	...
	this.getSetsPaged = function (start, maxRows) {
		$.getJSON('/Sets/SetHandler.ashx?rqtype=getsets&maxsets=10&UserID=' + DisplayedUserID, function (response) {
	...
	this.getSubscribedSetsPaged = function (start, maxRows) {
		$.getJSON('/Sets/SetHandler.ashx?rqtype=getsubscribedsets&maxsets=10&UserID=' + DisplayedUserID, function (response) {
	...
	this.getSetAssetImageThumbnail = function (setObj) {
		$.get('/Thumbs/RawAsset.ashx?AssetID=' + setObj.ImageAssetID + '&Width=75&Height=75&ImageFormat=png', function (data) {
			if (data !== null) {
				if (data == 'PENDING') {
					$('.AssetThumbnail .' + setObj.ImageAssetID).attr('src', '/images/spinners/spinner16x16.gif');
					window.setTimeout(function () {
						Roblox.SetsPaneObject.getSetAssetImageThumbnail(setObj);
					}, 2000);
					return;
				}
	...
	this.toggleBetweenSetsOwnedSubscribed = function () {
		...
		if (!Roblox.SetsPaneObject.isVisible(subscribedSetsElem)) {
			//turn subscribed on, turn off owned
