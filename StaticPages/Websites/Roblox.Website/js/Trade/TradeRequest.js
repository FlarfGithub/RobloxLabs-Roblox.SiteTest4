	...
	Roblox.Trade.TradeRequestModal = (function () {
		...
		$(function () {
			...
			_ModalElement.find('#ButtonCounterTrade').live('click', function () {
				close();
				window.open(
					'/Trade/TradeWindow.aspx?TradeSessionId=' + _TradeID + '&TradePartnerID=' + _PartnerID,
					'_blank',
			...
			_ModalElement.find('#ButtonDeclineTrade').live('click', function () {
				...
				Roblox.GenericConfirmation.open({
					...
					onDecline: openHelper,
					footerText:
						'Tired of lowball trades?<br />Update your Trade Quality setting on the <a href="' +
						Roblox.Endpoints.getAbsoluteUrl('/my/account#!/privacy') +
						'">Privacy page</a> within My Settings.',
		...
		function process(JSON) {
			...
			while (typeof tradeJSON.AgentOfferList[offerAgentCounter] !== 'undefined') {
				...
				if (agentOffer.AgentID === Number(_UserID)) offerIndex = 0;
				else offerIndex = 1;
				...
				if (typeof agentOffer.OfferRobux != 'undefined') {
					if (agentOffer.OfferRobux === 0) {
						...
						if (offerIndex === 1) {
							...
							var afterFeeRobux =
								Number(agentOffer.OfferRobux) - Math.ceil(Number(agentOffer.OfferRobux) * Number(_TradeSystemRobuxFee));
		...
		function messagingHelper(status, active) {
			...
			_ModalElement
				.find('p.TradeRequestText .TradePartnerName')
				.attr('href', '/users/' + _PartnerID + '/profile')
				.text(_TradePartner);
		...
		function expirationHelper(expirationDate) {
			...
			var inText = 'in ',
				...
				soonText = ' soon.';
			//</sl:translate>
			if (days > 1) _ModalElement.find('span#TradeRequestExpiration').text(inText + days + dayText);
			else if (hours > 1) _ModalElement.find('span#TradeRequestExpiration').text(inText + hours + hourText);
			else {
						...
						if (json.success === true) {
							if (requestCommand === _Command.pull) process(json.data);
								...
								var acceptMessage =
									'You have accepted ' +
									_TradePartner +
									"'s trade request. The trade is now being processed by our system.";
								close();
