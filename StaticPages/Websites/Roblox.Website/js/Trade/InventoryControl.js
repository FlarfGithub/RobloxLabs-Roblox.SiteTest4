...
Roblox.InventoryControl = (function () {
	...
	return function (elementId, assetTypeId, pageSize) {
		...
		var pager;
		var membershipLabels = [null, container.data('bc-label'), container.data('tbc-label'), container.data('obc-label')];
				...
				if (data.length > 0) {
					...
					data.forEach(function (item) {
						...
						var mockItem = {
							...
							ImageLink: Roblox.Endpoints.getAbsoluteUrl(
								'/asset-thumbnail/image?assetId=' + item.assetId + '&height=110&width=110',
							),
