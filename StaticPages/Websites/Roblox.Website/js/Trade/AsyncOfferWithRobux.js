...
if (typeof Roblox.Trade.Offer === 'undefined') {
	Roblox.Trade.Offer = (function () {
		var _TradePartnerID,
			...
			_YourRobuxRequest,
			_TradeSessionID,
			_TradeSystemMaxRobuxPercent,
			_TradeSystemCurrencyFee;
			...
			for (j = 0; j <= _OfferItems.length; j++) {
				for (i = 0; i <= inventoryItems.length; i++) {
					...
					if ($(inventoryItem).attr('userassetid') === _OfferItems[j]) {
						$(inventoryItem)
							.find('.TradeItemSilverButton')
							.addClass('TradeItemSilverButtonDisabled')
							.removeClass('TradeItemSilverButton');
						if (inventoryElement.attr('ownedbyuser') === 'True') {
		...
		function enableButton(inventoryItem) {
			...
			inventoryItem
				.find('a.TradeItemSilverButtonDisabled')
				.addClass('TradeItemSilverButton')
				.removeClass('TradeItemSilverButtonDisabled');
						...
						if (json.success) {
							if (dataSet.cmd === _SendTrade || dataSet.cmd === _CounterOffer) {
								...
								Roblox.GenericModal.open(
									tradeSentTitle,
									Roblox.Trade.Offer.Resources.thumbsUpUrl,
									tradeSentBody,
									function () {
										window.close();
							...
							if (json.data !== '') {
								Roblox.GenericModal.open(
									badNewsTitle,
									Roblox.Trade.Offer.Resources.alertUrl,
									json.msg + badNewsBody,
									handleDoubleItemsFaliure(json.data),
								);
							} else {
		...
		function addRequestHelper(buttonElement, list, index) {
			...
			if (!$(buttonElement).hasClass('TradeItemSilverButtonContainer'))
				$(buttonElement).removeClass('AddButton').removeClass('RequestButton');
			...
			if (list.length < _MaxItemsPerOffer) {
				list.push(inventoryItem);
			}
			//<sl:translate>
			else {
				errorElementTextDisplay(Roblox.Trade.Offer.Resources.maxItemsErrorText);
			}
			//</sl:translate>
		...
		function buildOfferJSON() {
			...
			if (isNaN(yourRobuxOffer)) yourRobuxOffer = 0;

			var yourRobuxRequest = parseInt(_YourRobuxRequest, 10);
			if (isNaN(yourRobuxRequest)) yourRobuxRequest = 0;
