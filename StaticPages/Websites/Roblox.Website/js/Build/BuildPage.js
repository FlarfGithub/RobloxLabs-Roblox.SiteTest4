...
$(function () {
	...
	placeCreationContextDropDown.change(function () {
		resetAndFetchCreationContexts(true, $('#MyCreationsTab'));
	});
	groupPlaceCreationContextDropDown.change(function () {
		resetAndFetchCreationContexts(true, $('#GroupCreationsTab'));
	});
	...
	$('.BuildPageContent').on('click', 'a.roblox-edit-button', function () {
		if ($('.build-page').data('edit-opens-studio') == 'False' && !Roblox.Client.isIDE()) {
			...
			Roblox.GenericConfirmation.open({
				...
				bodyContent:
					Roblox.BuildPage.Resources.openIn +
					"<a target='_blank' href='http://wiki.roblox.com/index.php/Studio'>" +
					Roblox.BuildPage.Resources.robloxStudio +
					'</a>.',
		...
		if (
			Roblox.Client.WaitForRoblox(function () {
				...
				RobloxLaunch.StartGame(
					'http://www.roblonium.com//Game/visit.ashx?placeId=' + play_placeId,
					'visit.ashx',
					'https://www.roblonium.com//Login/Negotiate.ashx',
					'FETCH',
					true,
				);
			})
		...
		$.post(url, reqData, function (data) {
			if (data.success) {
				...
				Roblox.GenericConfirmation.open({
					...
					onAccept: function () {
						processAdBuyPurchase(currentAdId, bidAmt, true, useGroupFundsChk, amountHtml);
					},
					allowHtmlContentInBody: true,
					dismissable: true,
				});
	...
	$('.items-container').on('click', 'a.process-ad-buy', function () {
		...
		var amountHtml =
			isNewRobuxIconEnabled === 'True' ? "<span class='icon-robux-16x16'></span><span>" : "<span class='currency CurrencyColor1'>";
		...
		var impressionToolTip =
			'<img class="tooltip-bottom"  src="' +
			Roblox.BuildPage.Resources.questionmarkImgUrl +
			'" alt="Help" title="' +
			Roblox.BuildPage.Resources.estimatorExplanation +
			'"/>';
		var impressionContainer = '';
		if (costPerImpression != '') {
			impressionContainer =
				'<br />' +
				Roblox.BuildPage.Resources.estimatedImpressions +
				impressionToolTip +
				': ' +
				Math.round(bidAmount / costPerImpression);
		}
		...
		Roblox.GenericConfirmation.open({
			...
			declineColor: Roblox.GenericConfirmation.gray,
			onAccept: function () {
				processAdBuyPurchase(adId, bidAmount, false, useGroupFundsChk, amountHtml);
			},
			onDecline: '',
			allowHtmlContentInBody: true,
			dismissable: true,
	...
	$('a.item-image.ad-image').click(function () {
		...
		var bannerPosition = ['30%', '30%'],
			skyscraperPosition = ['10%', '45%'],
			boxPosition = ['30%', '40%'];
