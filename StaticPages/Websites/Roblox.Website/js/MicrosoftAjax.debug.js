...
Function._validateParameterType = function Function$_validateParameterType(
	param,
	expectedType,
	expectedInteger,
	expectedDomElement,
...
Boolean.parse = function Boolean$parse(value) {
	...
	/// <returns type="Boolean"></returns>
	var e = Function._validateParams(arguments, [{ name: 'value', type: String }], false);
	if (e) throw e;
...
Type._checkDependency = function Type$_checkDependency(dependency, featureName) {
	...
	if (typeof featureName !== 'undefined' && !isDependent) {
		throw Error.invalidOperation(String.format(Sys.Res.requiredScriptReferenceNotIncluded, featureName, dependency));
	}
	return isDependent;
};
...
Sys.NotifyCollectionChangedEventArgs = function Sys$NotifyCollectionChangedEventArgs(changes) {
	...
	var e = Function._validateParams(arguments, [{ name: 'changes', type: Array, elementType: Sys.CollectionChange }]);
	if (e) throw e;
...
Sys.Observer.makeObservable = function Sys$Observer$makeObservable(target) {
	...
	/// <returns></returns>
	var e = Function._validateParams(arguments, [{ name: 'target' }]);
	if (e) throw e;
...
Sys.Observer.addPropertyChanged = function Sys$Observer$addPropertyChanged(target, handler) {
	...
	/// <param name="target" mayBeNull="false"></param>
	/// <param name="handler" type="Function"></param>
	var e = Function._validateParams(arguments, [{ name: 'target' }, { name: 'handler', type: Function }]);
	if (e) throw e;
	Sys.Observer._ensureObservable(target);
...
Sys.Observer.clear = function Sys$Observer$clear(target) {
	...
	/// <param name="target" type="Array" elementMayBeNull="true"></param>
	var e = Function._validateParams(arguments, [{ name: 'target', type: Array, elementMayBeNull: true }]);
	if (e) throw e;
...
Date.prototype._toFormattedString = function Date$_toFormattedString(format, cultureInfo) {
	...
	for (;;) {
		...
		switch (ar[0]) {
			...
			case 'MMMM':
				ret.append(
					dtf.MonthGenitiveNames && hasDay() ? dtf.MonthGenitiveNames[getPart(this, 1)] : dtf.MonthNames[getPart(this, 1)],
				);
				break;
			case 'MMM':
				ret.append(
			...
			case 'yy':
				ret.append(
					addLeadingZero((converted ? converted[0] : Date._getEraYear(this, dtf, Date._getEra(this, eras), sortable)) % 100),
				);
				break;
			case 'y':
	...
	function expandNumber(number, precision, groupSizes, sep, decimalChar) {
		...
		while (stringIndex >= 0) {
			if (curSize === 0 || curSize > stringIndex) {
				if (ret.length > 0) return numberString.slice(0, stringIndex + 1) + sep + ret + right;
				else return numberString.slice(0, stringIndex + 1) + right;
			}
			if (ret.length > 0) ret = numberString.slice(stringIndex - curSize + 1, stringIndex + 1) + sep + ret;
			else ret = numberString.slice(stringIndex - curSize + 1, stringIndex + 1);
			stringIndex -= curSize;
...
Sys.Serialization.JavaScriptSerializer._serializeBooleanWithBuilder = function Sys$Serialization$JavaScriptSerializer$_serializeBooleanWithBuilder(
	object,
	stringBuilder,
) {
	stringBuilder.append(object.toString());
};
Sys.Serialization.JavaScriptSerializer._serializeNumberWithBuilder = function Sys$Serialization$JavaScriptSerializer$_serializeNumberWithBuilder(
	object,
	stringBuilder,
) {
	if (isFinite(object)) {
...
Sys.Serialization.JavaScriptSerializer._serializeStringWithBuilder = function Sys$Serialization$JavaScriptSerializer$_serializeStringWithBuilder(
	string,
	stringBuilder,
) {
...
Sys.Serialization.JavaScriptSerializer._serializeWithBuilder = function Sys$Serialization$JavaScriptSerializer$_serializeWithBuilder(
	object,
	stringBuilder,
	sort,
	prevObjects,
...
Sys.Serialization.JavaScriptSerializer.deserialize = function Sys$Serialization$JavaScriptSerializer$deserialize(data, secure) {
	...
	try {
		...
		if (
			secure &&
			Sys.Serialization.JavaScriptSerializer._jsonRegEx.test(exp.replace(Sys.Serialization.JavaScriptSerializer._jsonStringRegEx, ''))
		)
			throw null;
		return eval('(' + exp + ')');
	} catch (e) {
...
Sys.UI.DomElement.isDomElement = function Sys$UI$DomElement$isDomElement(obj) {
	...
	/// <returns type="Boolean"></returns>
	var e = Function._validateParams(arguments, [{ name: 'obj' }]);
	if (e) throw e;
...
Sys.UI.DomElement.removeCssClass = function Sys$UI$DomElement$removeCssClass(element, className) {
	...
	if (index >= 0) {
		element.className = (
			currentClassName.substr(0, index) +
			' ' +
...
Sys.UI.DomElement._getCurrentStyle = function Sys$UI$DomElement$_getCurrentStyle(element) {
	...
	var computedStyle =
		w && element !== w && w.getComputedStyle ? w.getComputedStyle(element, null) : element.currentStyle || element.style;
...
function Sys$_Application$disposeElement(element, childNodesOnly) {
	...
	var e = Function._validateParams(arguments, [{ name: 'element' }, { name: 'childNodesOnly', type: Boolean }]);
	if (e) throw e;
...
function Sys$UI$Behavior$set_name(value) {
	var e = Function._validateParams(arguments, [{ name: 'value', type: String }]);
	if (e) throw e;
	if (value === '' || value.charAt(0) === ' ' || value.charAt(value.length - 1) === ' ') throw Error.argument('value', Sys.Res.invalidId);
	if (typeof this._element[value] !== 'undefined') throw Error.invalidOperation(String.format(Sys.Res.behaviorDuplicateName, value));
...
Sys.Net.WebServiceProxy.invoke = function Sys$Net$WebServiceProxy$invoke(
	...
	var schemeHost = enableJsonp !== false ? Sys.Net.WebServiceProxy._xdomain.exec(servicePath) : null,
		tempCallback,
		jsonp = schemeHost && schemeHost.length === 3 && (schemeHost[1] !== location.protocol || schemeHost[2] !== location.host);
	useGet = jsonp || useGet;
	if (jsonp) {
	...
	var script,
		...
		loader,
		body = null,
		url = Sys.Net.WebRequest._createUrl(
			methodName ? servicePath + '/' + encodeURIComponent(methodName) : servicePath,
			urlParams,
			jsonp ? jsonpCallbackParameter + '=Sys.' + tempCallback : null,
		);
	if (jsonp) {
		...
		function jsonpComplete(data, statusCode) {
			...
			if (typeof statusCode !== 'undefined' && statusCode !== 200) {
				if (onFailure) {
					error = new Sys.Net.WebServiceError(
						false,
					...
					throw Sys.Net.WebServiceProxy._createFailedError(
						methodName,
...
Sys.Res = {
	...
	addHandlerCantBeUsedForError:
		"Can't add a handler for the error event using this method. Please set the window.onerror property instead.",
