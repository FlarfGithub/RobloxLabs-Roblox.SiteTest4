...
if (typeof Roblox.AddEmail === 'undefined') {
	Roblox.AddEmail = (function () {
		...
		function loadEmailModal(showParent) {
			...
			var bodyContents =
				"<div id='invalidEmail' style='display: none' class='validation-summary-errors'><span class='text'>" +
				invalidEmailText +
				"</span></div><div id='noEmail' style='display: none' class='validation-summary-errors'><span class='text'>" +
				noEmailText +
				"</span></div><div id='noPassword' style='display: none' class='validation-summary-errors'><span class='text'>" +
				noPasswordText +
				"</span></div><div id='StandardError' style='display: none' class='validation-summary-errors'><span class='text'>" +
				standardErrorText +
				"</span></div><table style='margin: auto'><tr><td><label class='form-label'>" +
				emailText +
				...
				passwordText +
				"</label></td><td><input id='PasswordTextBox' type='password' class='text-box text-box-large emailInput emailChangeMonitor' /></td></tr></table>" +
				"<div id='Processing' style='display:none;margin-bottom: 10px;'><img src='/images/Accounts/ProgressIndicator.gif' alt='" +
				processingText +
				"'/></div>";
			...
			if (validateEmail()) {
				var onSuccess = (onError = function (response) {
					if (response.Result) {
						...
						if (!RobloxMissingParentEmail.resetParentEmail) reloadPageOnSuccess = true;
						Roblox.GenericConfirmation.open({
				...
				Roblox.Website.Services.Secure.AddParentEmail.InsertParentEmail(
					email,
					uID,
					password,
					userIP,
					RobloxMissingParentEmail.resetParentEmail,
					onSuccess,
					onError,
				);
				$('.ConfirmationModalButtonContainer a.btn-neutral').addClass('btn-disabled-neutral');
				$('.ConfirmationModalButtonContainer a.btn-disabled-neutral').click(function () {
					return false;
				});
				$('.ConfirmationModalButtonContainer a.btn-negative').addClass('btn-disabled-negative');
				$('.ConfirmationModalButtonContainer a.btn-disabled-negative').click(function () {
					return false;
				});
				return false;
			} else {
