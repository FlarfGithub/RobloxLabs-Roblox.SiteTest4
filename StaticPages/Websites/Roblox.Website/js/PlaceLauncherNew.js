...
var RobloxPlaceLauncherService = {
	...
	RequestFollowUser: function (userId, onGameSuccess, onGameError, context) {
		$.getJSON('/Game/PlaceLauncher.ashx', { request: 'RequestFollowUser', userId: userId }, function (data) {
			if (data.Error) {
				onGameError(data.Error, context);
			} else {
				onGameSuccess(data, context);
...
RobloxLaunch.StartGame = function (visitUrl, type, authenticationUrl, authenticationTicket) {
	...
	if (RobloxLaunch._GoogleAnalyticsCallback != null) RobloxLaunch._GoogleAnalyticsCallback();
	...
	try {
		...
		} else urchinTracker('Visit/Fail/' + prefix + encodeURIComponent(message));
...
RobloxLaunch.CheckGameStarted = function (launcher) {
	$('#PlaceLauncherStatusPanel').modal({ escClose: true, opacity: 80, overlayCss: { backgroundColor: '#000' } });
...
RBX.PlaceLauncher.prototype = {
	...
	RequestGameJob: function (placeId, gameId, gameJobId) {
		...
		var self = this;
		var gameDelegate = function () {
			RobloxPlaceLauncherService.RequestGameJob(placeId, gameId, gameJobId, onGameSuccess, onGameError, self);
		};
	...
	_startUpdatePolling: function (joinGameDelegate) {
		try {
			...
			if (window.ActiveXObject) var result = launcher.IsUpToDate;
	...
	_showDialog: function () {
		...
		$(this._popup).modal({ escClose: true, opacity: 80, overlayCss: { backgroundColor: '#000' } });
