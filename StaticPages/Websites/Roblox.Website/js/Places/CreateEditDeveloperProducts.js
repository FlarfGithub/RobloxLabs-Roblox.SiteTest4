$(function () {
	...
	Roblox.DeveloperProductsForm.init = function () {
		...
		$('body')
			.unbind('developerProductImageIFrameLoaded')
			.bind('developerProductImageIFrameLoaded', function (evt, data) {
				Roblox.DeveloperProductsForm.onImageUploadComplete(true, data);
			});

		$('body')
			.unbind('developerProductImageIFrameLoadError')
			.bind('developerProductImageIFrameLoadError', function (evt, data) {
		...
		$('#AddDeveloperProduct')
			.off('click')
			.on('click', 'a.developer-product-button', function (event) {
	...
	Roblox.DeveloperProductsForm.validateAndGetFormValues = function () {
		...
		if (
			...
			robuxPriceErrorMessage.length > 0 ||
			$developerProductIconIdElement.attr('uploaded') == 'false'
		) {
			formValues = null;
		} else {
