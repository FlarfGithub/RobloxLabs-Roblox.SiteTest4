	...
	function initializeChosen() {
		...
		if ($('#GamePlaceAccess').data('permissions-service-enabled') === 'True' && $('.chzn-container').length <= 0) {
			$('.chosen').chosen({
				...
				max_selected_options: maxUsersAllowedInList,
				disable_search: true,
				disable_search_threshold: 1,
			});
			$('.chosen').bind('liszt:maxselected', function () {
...
$(function () {
	...
	$('#NumPlayers').on('change', '#MaxPlayersInput', function () {
		...
		for (i = min; i < max; i++) {
			$(friendSlotInput).append($('<option></option>').attr('value', i).text(i));
		}

		for (i = min; i <= max; i++) {
			$(preferredPlayersInput).append($('<option></option>').attr('value', i).text(i));
		}
	...
	$('#playerAccessTab').one('click', function () {
		...
		$.ajax({
			...
			.done(function (jsonData) {
				...
				container.text(
					'There are currently ' +
						jsonData.activeSubscriptionsCount +
						' VIP Server Subscriptions active for this place. Changing the price will cancel all of these subscriptions.',
				);
			})
			.fail(function () {
				container.hide();
