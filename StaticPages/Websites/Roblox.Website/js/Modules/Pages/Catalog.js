Roblox.define(
	'Pages.Catalog',
	['Widgets.ItemImage', 'Widgets.HierarchicalDropdown', 'Pages.CatalogShared'],
	function (itemImage, dropdown) {
		...
		function init(pagestateInit, totalNumberOfPagesInit, saveFilters) {
			...
			$('.assetTypeFilter').on('click', function () {
				...
				if (tryCategory !== undefined) {
					if (tryKeepFilters !== undefined) Clear({ types: true, category: true });
					else ClearAll();
			...
			$('.genreFilter').click(function () {
				pagestate.Genres = $('input.genreFilter:checked')
					.map(function () {
		...
		function ChangePageOnPagingInput() {
			...
			if (pagestate.PageNumber >= 1) {
				if (pagestate.PageNumber > pagestate.TotalNumberOfPages) pagestate.PageNumber = pagestate.TotalNumberOfPages;
				UpdateCatalog(true);
			}
		}
		...
		function UpdateCatalog(rememberPage) {
			...
			if (pagestate.Subcategory != null && pagestate.Subcategory != '') url += 'Subcategory=' + pagestate.Subcategory + '&';
