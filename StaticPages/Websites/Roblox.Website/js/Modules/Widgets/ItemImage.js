Roblox.define('Widgets.ItemImage', [], function () {
	...
	function onload(img, data) {
		...
		data.bcOverlayUrl != null &&
			((bcOverlay = $('<img>')
				.attr('src', data.bcOverlayUrl)
				...
				.attr('border', 0)),
			img.after(bcOverlay)),
			data.limitedOverlayUrl != null &&
				((limitedOverlay = $('<img>')
					.attr('alt', data.limitedAltText)
					.css('position', 'absolute')
					.css('left', '0')
					.css('bottom', '0')
					.attr('border', 0)),
			...
			data.deadlineOverlayUrl != null &&
				((deadlineOverlay = $('<img>').attr('alt', 'Deadline').attr('border', 0)),
				deadlineOverlay.attr('src', data.deadlineOverlayUrl),
				img.after(deadlineOverlay)),
			data.iosOverlayUrl != null
				? ((iosOverlay = $('<img>').attr('alt', 'iOS Only').attr('border', 0)),
				  iosOverlay.attr('src', data.iosOverlayUrl),
				  img.after(iosOverlay))
				: data.saleOverlayUrl != null
				? ((saleOverlay = $('<img>').attr('alt', 'Sale').attr('border', 0)),
				  saleOverlay.attr('src', data.saleOverlayUrl),
				  img.after(saleOverlay))
				: data.newOverlayUrl != null &&
				  ((newOverlay = $('<img>')
						.attr('alt', 'New')
						.attr('border', 0)
	...
	function load(elements, attempt) {
		if ($.type(elements) !== 'array') elements = [elements];
							...
							if (params[i].imageSize !== 'custom') {
								if (Roblox.GlobalImageSizes) {
									var img = $('<img>')
										...
										.attr('alt', data.name)
										.attr('border', 0)
										.attr('height', Roblox.GlobalImageSizes[params[i].imageSize])
										.attr('width', Roblox.GlobalImageSizes[params[i].imageSize])
										.addClass('original-image modal-thumb');
								} else {
									var img = $('<img>')
										.attr('title', data.name)
										.attr('alt', data.name)
								...
								var img = $('<img>')
									...
									.attr('alt', data.name)
									.attr('border', 0)
									.attr('height', params[i].imageSizeY)
									.attr('width', params[i].imageSizeX)
									.addClass('original-image modal-thumb');
							}
							img.load(
								(function (innerDiv, element, img, data) {
									return function () {
