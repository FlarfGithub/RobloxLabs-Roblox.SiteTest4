Roblox.define('Widgets.DropdownMenu', [], function () {
	function LazyInitializeDropdown(parentSelector) {
		$(parentSelector).on('click', '.button', function () {
			...
			if (!target.hasClass('init')) {
				var buttonWidth =
					$(this).outerWidth() - parseInt(target.css('border-left-width')) - parseInt(target.css('border-right-width'));
	...
	function InitializeDropdown() {
		...
		browseButton.each(function () {
			var buttonWidth =
				$(this).outerWidth() - parseInt($(this).css('border-left-width')) - parseInt($(this).css('border-right-width'));
