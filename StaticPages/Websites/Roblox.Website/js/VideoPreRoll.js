...
Roblox.VideoPreRoll = {
	...
	videoOptions: {
		...
		companionId:
			'videoPrerollCompanionAd' /*,     // The id attribute of the div to contain the companion ad
	...
	start: function start() {
		...
		if (Roblox.VideoPreRoll.specificAdOnPlacePageEnabled && typeof play_placeId !== 'undefined') {
			...
			if (
				play_placeId == Roblox.VideoPreRoll.specificAdOnPlacePageId &&
				Roblox.VideoPreRoll.videoOptions.categories.indexOf(categoryPlusComma) == -1
			) {
		...
		if (Roblox.VideoPreRoll.specificAdOnPlacePage2Enabled && typeof play_placeId !== 'undefined') {
			...
			if (
				play_placeId == Roblox.VideoPreRoll.specificAdOnPlacePage2Id &&
				Roblox.VideoPreRoll.videoOptions.categories.indexOf(categoryPlusComma) == -1
			) {
				Roblox.VideoPreRoll.videoOptions.categories += categoryPlusComma;
			}
		...
		this.loadingBarIntervalID = setInterval(function () {
			...
			LoadingBar.update(
				Roblox.VideoPreRoll.loadingBarID,
				Roblox.VideoPreRoll.loadingBarCurrentTime / Roblox.VideoPreRoll.loadingBarMaxTime,
			);
		}, loadingBarInterval);
        ...
        */
		...
		this.myvpaidad.on(Event.AdLoaded, function (e) {
			Roblox.VideoPreRoll._onVideoLoaded(e);
		});
		this.myvpaidad.on(Event.AdStarted, function (e) {
			Roblox.VideoPreRoll._onVideoStart(e);
		});
		this.myvpaidad.on(Event.AdStopped, function (e) {
			Roblox.VideoPreRoll._onVideoComplete(e);
		});
		this.myvpaidad.on(Event.AdError, function (e) {
			Roblox.VideoPreRoll._onVideoError(e);
		});
	...
	_onVideoError: function _onVideoError(e) {
		// theoretically, e is an event object from adap.tv
	...
	isPlaying: function isPlaying() {
		...
		if (
			Roblox.VideoPreRoll.videoStarted &&
			!Roblox.VideoPreRoll.videoCompleted &&
	...
	test: function openVideoPreroll2(options) {
		...
		if (!MadStatus.running) {
			MadStatus.init(
				$('#videoPrerollPanel').find('.MadStatusField'),
				$('#videoPrerollPanel').find('.MadStatusBackBuffer'),
				2000,
				800,
			);
			MadStatus.start();
