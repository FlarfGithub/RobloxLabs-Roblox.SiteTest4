...
Roblox.PushNotificationRegistrationUI = (function () {
	...
	var promptToAllowPush = function (customMessage, ignoreTimeSinceLastPrompt) {
		...
		if (
			$('.push-notifications-global-prompt .alert-info').length === 0 &&
			($('.alert-info').length > 0 || $('.alert-cookie-notice').length > 0)
		) {
			// site-wide banner is already being displayed, should not show our prompt
			return;
		}

		$.when(isAlreadyRegistered(), isOptedOutOfDesktopPush())
			.done(function (isRegistered, isOptedOut) {
				if (isRegistered || isOptedOut) {
					...
					if (!ignoreTimeSinceLastPrompt) {
						Roblox.PushNotificationRegistrar.getEventPublisher().Publish(
							Roblox.PushNotificationEventPublishers.RegistrationEventTypes.promptShown,
						);
					}
	...
	var isOptedOutOfDesktopPush = function () {
		...
		$.ajax({
			...
			success: function (response) {
				if (
					response &&
					response.optedOutReceiverDestinationTypes &&
					response.optedOutReceiverDestinationTypes.indexOf('DesktopPush') !== -1
				) {
	...
	var declinePrompt = function () {
		Roblox.PushNotificationRegistrar.getEventPublisher().Publish(
			Roblox.PushNotificationEventPublishers.RegistrationEventTypes.promptDeclined,
		);
		clearAllPromptsAndModals();
	...
	var acceptPrompt = function () {
		Roblox.PushNotificationRegistrar.getEventPublisher().Publish(
			Roblox.PushNotificationEventPublishers.RegistrationEventTypes.promptAccepted,
		);
	...
	var initialize = function () {
		...
		promptIntervalTracker = new Roblox.PersistedBackoffIntervalTracker(
			pushPromptIntervals,
			'Roblox.PushNotificationRegistrationUI.Prompt.' + userId,
		);
