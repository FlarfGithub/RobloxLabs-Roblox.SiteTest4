...
function DataPager(
	...
	itemContainerDivID,
	pagerContainerDivID,
	GetItemsPaged_Function,
	ItemFormatter_Function,
	ItemFormatterCallback_Function,
	pagerOptions,
) {
	...
	var callUpdatePager = function (newTotal, newPage) {
		...
		if (maxPage != Math.ceil(totalItems / itemsPerPage) || totalItems == itemsPerPage || totalItems == 0) {
			//if the number of pages has changed, redraw pager
	...
	function drawPager() {
		...
		var pagerspan = $('<span></span>').append(
			'<a class="disabled pager first" pagingaction="First">First</a><a class="disabled pager previous" pagingaction="Previous">Previous</a>',
		);
		if (options.Paging_PageNumbers_AreLinks) {
			...
			pagerspan = $('<span></span>').append(
				'<a class="disabled pager first" pagingaction="First"></a><a class="disabled pager previous" pagingaction="Previous"></a>',
			);
			pagerspan.append(
				'<span class="page text">Page <span class="CurrentPage">1</span> of <span class="TotalPages">' + maxPage + '</span></span>',
			);
