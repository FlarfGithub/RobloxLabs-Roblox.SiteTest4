...
Roblox.ItemPurchase = function (purchaseConfirmationCallback) {
	...
	function formatMoney(money) {
		...
		if (money < 10000) return addCommasToMoney(money);
	...
	function purchaseItem(elem, CssClass) {
		...
		var target = $(elem);
		if (target.hasClass('btn-disabled-primary')) return;
		...
		$.ajax({
			type: 'POST',
			url:
				'/API/Item.ashx?rqtype=purchase&productID=' +
				productID +
				'&expectedCurrency=' +
				expCurrency +
				'&expectedPrice=' +
	...
	function openErrorView(errorObj) {
		if (errorObj.showDivID === 'TransactionFailureView') {
			...
			if (errorObj.currentCurrency == 1) {
				...
				footer =
					Roblox.ItemPurchase.strings.orText +
					" <a href='/My/Money.aspx?tab=TradeCurrency' style='font-weight:bold'>" +
					Roblox.ItemPurchase.strings.tradeCurrencyText +
					'</a>';
			}
			...
			Roblox.GenericConfirmation.open({
				...
				bodyContent: String.format(
					Roblox.ItemPurchase.strings.insufficientFundsText,
					"<span class='currency-color CurrencyColor" + errorObj.currentCurrency + "'>" + errorObj.shortfallPrice + '</span>',
				),
			...
			var elem = $(
				'.PurchaseButton[data-item-id=' +
					errorObj.AssetID +
					'][data-expected-currency=' +
					errorObj.expectedCurrency +
					'][data-expected-price=' +
					errorObj.expectedPrice +
					']',
			);
			...
			Roblox.GenericConfirmation.open({
				...
				bodyContent: String.format(
					Roblox.ItemPurchase.strings.priceChangeText,
					"<span class='currency-color CurrencyColor" + errorObj.expectedCurrency + "'>" + errorObj.expectedPrice + '</span>',
					"<span class='currency-color CurrencyColor" + errorObj.currentCurrency + "'>" + errorObj.currentPrice + '</span>',
				),
				...
				footerText: String.format(
					Roblox.ItemPurchase.strings.balanceText,
					(errorObj.currentCurrency == 1 ? 'R$' : 'Tx') + errorObj.balanceAfterSale,
				),
				allowHtmlContentInBody: true,
				dismissable: true,
			});
	...
	function openPurchaseVerificationView(elem) {
		...
		Roblox.GenericConfirmation.open({
			...
			bodyContent: String.format(
				Roblox.ItemPurchase.strings.buyItemText,
				buyType,
				assetName,
				assetType,
				sellerName,
				currencyContent,
				isPlace ? Roblox.ItemPurchase.strings.accessText : '',
			),
			imageUrl: itemUrl,
	...
	function openPurchaseConfirmationView(obj) {
		...
		if (obj.AssetIsWearable) {
			footer =
				"<a class='CustomizeCharacterLink' href='/My/Character.aspx'>" +
				Roblox.ItemPurchase.strings.customizeCharacterText +
				'</a>';
		...
		Roblox.GenericConfirmation.open({
			...
			bodyContent: String.format(
				Roblox.ItemPurchase.strings.purchaseCompleteText,
				obj.TransactionVerb,
				obj.AssetName,
				obj.AssetType,
				obj.SellerName,
