...
function addYears() {
	for (var n = new Date().getFullYear(); n >= new Date().getFullYear() - 100; n--)
		$('<option>').attr('value', n).text(n).appendTo('select[name="Year"]');
}
$(function () {
	addDays(),
		addMonths(),
		addYears(),
		$('[roblox-js-onclick]').click(function () {
			return $('#loginForm').submit(), !1;
		}),
		$('[roblox-js-onsignup]').click(function () {
			var i = $('select[name="Month"] option:selected').val(),
				r = $('select[name="Day"] option:selected').val(),
				u = $('select[name="Year"] option:selected').val(),
				n = $('#ReturnUrl').val(),
				t = String.format('/account/signupredir?month={0}&day={1}&year={2}', i, r, u);
		...
		$('#MonthSelect').change(function () {
			var i = $('select[name="Month"] option:selected').val(),
				r = $('select[name="Day"] option:selected').val(),
				n = $('select[name="Year"] option:selected').val(),
				t = String.format('/IDE/Login?isSignup=true&month={0}&day={1}&year={2}', i, r, n);
		...
		$('#errorBanner').text().trim().length == 0 && $('#errorBanner').hide(),
		$('#Username').focus(),
