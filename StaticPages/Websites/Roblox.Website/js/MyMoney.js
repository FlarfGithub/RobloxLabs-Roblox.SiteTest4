$(function () {
				...
				for (var summaryIndex in summaryData) {
					...
					if (typeof robux === 'number') {
						...
						if (summaryClassMap.hasOwnProperty(summaryIndex)) {
							summaryTab
								.find('.' + summaryClassMap[summaryIndex])
								.html(FormatSummaryNumber(Roblox.NumberFormatting.commas(robux)));
						}
					}
				}
	...
	var myTransactionsRowTemplate = $('<tr class="datarow"></tr>').append(
		'<td class="Date"></td><td class="Member"></td><td class="Description"></td><td class="Amount"><span>&nbsp;</span></td>',
	);
	var myTransactionsPaymentRowTemplate = $('<tr class="datarow paymentrow"></tr>').append(
		'<td class="Date"></td><td class="Member"></td><td class="Description"></td><td class="Amount"><span>&nbsp;</span></td>',
	);
	var noRecordsMessage = 'No records found.';
	var noSalesMessage = 'You have not sold any items!';
	var noPurchasesMessage =
		"You have not purchased any items! Browse the <a href='" +
		(Roblox && Roblox.Endpoints ? Roblox.Endpoints.getAbsoluteUrl('/Catalog') : '/Catalog') +
		"'>Catalog</a> to buy items.";
	...
	function CreateTransactionRow(obj, template) {
		...
		if (obj.Successful_Payments == 0 && obj.Has_Payments) {
			newRow
				.find('.Description')
				.prepend($("<a href='#'>\u25BE<a>").attr('onClick', '$(".payment' + obj.Sale_ID + '").toggle();return false;'));
		}
		...
		if (obj.Member_ID == '1') {
			...
			if (obj.MemberIsGroup == 'True') {
				newRow
					...
					.prepend(
						$('<div></div>')
							.attr('class', 'roblox-group-image')
							.attr('data-group-id', obj.Group_ID)
							.attr('data-image-size', 'small')
				...
				newRow
					...
					.prepend(
						$('<div></div>')
							.attr('class', 'avatar avatar-headshot-sm roblox-avatar-image')
							.attr('data-user-id', obj.Member_ID)
		...
		$.ajax({
			...
			success: function (msg) {
				...
				if (response.Data.length == 0) {
					...
					for (var i = 0; i < response.Data.length; i++) {
						if (transactiontype === 'devex') {
							$('.TransactionsContainer table').append(
								CreateImbursementRow($.parseJSON(response.Data[i]), myTransactionsRowTemplate),
							);
						} else {
				...
				if (myTransactionsStartIndex < totalCount) {
					...
					$('.TransactionsContainer table').append(
						'<tr class="morerecords"><td colspan=4><a class="btn-control btn-control-small">' +
							moreRecordsText +
							'</a></td></tr>',
					);
	...
	function CreateTradeItemRow(obj) {
		...
		newRow
			.find('.Action')
			.html(
				'<a class="text-link ViewTradeLink" tradePartnerID="' +
					obj.TradePartnerID +
					'" tradeSessionID="' +
					obj.TradeSessionID +
					'" >View Details</a>',
		...
		newRow
			.find('.TradePartner')
			.append($('<span class="text-overflow"></span>').text(obj.TradePartner))
			.attr('TradePartnerName', obj.TradePartner);
		newRow
			.find('.TradePartner')
			.prepend(
				$('<div></div>')
	...
	function LoadMyItemTrades() {
		...
		$.ajax({
			...
			success: function (msg) {
				var response = $.parseJSON(msg.d);
				var showTradeWriteDisabled =
					response.tradeWriteEnabled === 'False' && (tradetype === 'inbound' || tradetype === 'outbound');
				...
				if (response.Data.length == 0) {
					...
					//</sl:translate>
					$('.TradeItemsContainer table .loading')
						.html(notrades1 + tradetype + notrades2)
						.removeClass('loading')
						.addClass('empty');
				} else {
				...
				if ($('.TradeItemsContainer table .datarow').length < response.totalCount) {
					...
					var moreRecordsText = 'More Records';
					//</sl:translate>
					$('.TradeItemsContainer table').append(
						'<tr class="morerecords"><td colspan=4><a class="btn-control btn-control-small">' +
							moreRecordsText +
							'</a></td></tr>',
					);
