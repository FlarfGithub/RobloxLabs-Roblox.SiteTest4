...
var RobloxPlaceLauncherService = {
	...
	RequestGroupBuildGame: function (placeId, onGameSuccess, onGameError, context) {
		$.getJSON('/Game/PlaceLauncher.ashx', { request: 'RequestGroupBuildGame', placeId: placeId }, function (data) {
			if (data.Error) {
				onGameError(data.Error, context);
			} else {
				onGameSuccess(data, context);
	...
	RequestGameJob: function (placeId, gameId, onGameSuccess, onGameError, context) {
		$.getJSON('/Game/PlaceLauncher.ashx', { request: 'RequestGameJob', placeId: placeId, gameId: gameId }, function (data) {
			if (data.Error) {
				onGameError(data.Error, context);
			} else {
				onGameSuccess(data, context);
	...
	CheckGameJobStatus: function (jobId, onSuccess, onError, context) {
		$.getJSON('/Game/PlaceLauncher.ashx', { request: 'CheckGameJobStatus', jobId: jobId }, function (data) {
			if (data.Error) {
...
RobloxLaunch.StartGameWork = function (visitUrl, type, authenticationUrl, authenticationTicket, isEdit) {
	...
	if (RobloxLaunch.googleAnalyticsCallback !== null) RobloxLaunch.googleAnalyticsCallback();
	...
	try {
		if (typeof window.external !== 'undefined' && window.external.IsRoblox2App && (visitUrl.indexOf('visit') != -1 || isEdit)) {
			...
			if (!launcher) {
				...
				try {
					try {
						if (Roblox.Client.IsIE() || Roblox.Client._legacyLaunch == false) {
							// IE
...
RBX.PlaceLauncher.prototype = {
	_showDialog: function () {
		...
		if (this._popupID == 'PlaceLauncherStatusPanel') {
			if (Roblox.VideoPreRoll && Roblox.VideoPreRoll.showVideoPreRoll && !Roblox.VideoPreRoll.isExcluded()) {
				...
				_popupOptions.onClose = function (dialog) {
	...
	_updateStatus: function (status) {
		...
		$(this._popup)
			...
			.css('visibility', status === 3 || status === 4 || status === 5 ? 'hidden' : 'visible');
	},
	...
	_startUpdatePolling: function (joinGameDelegate) {
		try {
			...
			if (Roblox.Client.IsIE()) var result = launcher.IsUpToDate;
			else result = launcher.Get_IsUpToDate();
	...
	_onUpdateStatus: function (result, launcher, joinGameDelegate) {
		...
		if (result === 8) {
			...
			var self = this;
			var call = function () {
				self.CheckUpdateStatus(onSuccess, onError, launcher, joinGameDelegate, self);
			};
			window.setTimeout(call, 2000);
		} else {
	...
	RequestGame: function (placeId, gender) {
		...
		var onGameError = function (result, context) {
			context._onGameError(result);
		};
		var self = this;
		...
		var gameDelegate = function () {
			EventTracker.start('GetConnection');
			RobloxPlaceLauncherService.RequestGame(placeId, isPartyLeader, gender, onGameSuccess, onGameError, self);
		};
	...
	RequestPrivateGame: function (placeId, accessCode, gender) {
		...
		var gameDelegate = function () {
			EventTracker.start('GetConnection');
			RobloxPlaceLauncherService.RequestPrivateGame(placeId, accessCode, gender, onGameSuccess, onGameError, self);
		};
	...
	RequestPlayWithParty: function (placeId, partyGuid, gameId) {
		...
		var gameDelegate = function () {
			EventTracker.start('GetConnection');
			RobloxPlaceLauncherService.RequestPlayWithParty(placeId, partyGuid, gameId, onGameSuccess, onGameError, self);
		};
	...
	RequestGroupBuildGame: function (placeId) {
		...
		var gameDelegate = function () {
			EventTracker.start('GetConnection');
			RobloxPlaceLauncherService.RequestGroupBuildGame(placeId, onGameSuccess, onGameError, self);
		};
	...
	RequestFollowUser: function (userId) {
		...
		var onGameError = function (result, context) {
			context._onError(result);
		};
		var self = this;
		var gameDelegate = function () {
			EventTracker.start('GetConnection');
			RobloxPlaceLauncherService.RequestFollowUser(userId, onGameSuccess, onGameError, self);
		};
	...
	RequestGameJob: function (placeId, gameId) {
		...
		var gameDelegate = function () {
			EventTracker.start('GetConnection');
			RobloxPlaceLauncherService.RequestGameJob(placeId, gameId, onGameSuccess, onGameError, self);
		};
