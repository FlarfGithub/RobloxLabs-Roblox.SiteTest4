...
Roblox.Controls.Image.IE6Hack = function (a) {
	a = a.getElementsByTagName('img')[0];
	var b = a.src;
	a.src = a.blankUrl;
	a.runtimeStyle.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod='crop', src='" + b + "')";
};
Roblox.Controls.Image.SetImageSrc = function (a, b) {
	a = a.getElementsByTagName('img')[0];
	if (a.blankUrl) a.runtimeStyle.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod='crop', src='" + b + "')";
	else a.src = b;
};
Roblox.Controls.Image.IsBadImage = function (a) {
	if (!a.complete) return false;
	if (typeof a.naturalWidth != 'undefined' && a.naturalWidth === 0) return false;
	return true;
};
Roblox.Controls.Image.ErrorUrl = '';
Roblox.Controls.Image.OnError = function (a) {
	if (Roblox.Controls.Image.IsBadImage(a)) {
		var b = new XMLHttpRequest();
		a = a.src;
		b.open('POST', Roblox.Controls.Image.ErrorUrl, true);
		b.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		b.setRequestHeader('Content-length', a.length);
...
Roblox.Thumbs.Image.prototype = {
	...
	dispose: function () {
		if (typeof this._updateTimeout !== 'undefined') window.clearTimeout(this._updateTimeout);
	...
	_onUrl: function (result) {
		...
		if ((!result.final || result.url.indexOf('unavail') > 0) && this._waitTime <= 40) {
			// Give up after 40 times.
			// Try again later
...
Roblox.Thumbs.AssetImage.prototype = {
	...
	requestThumbnail: function () {
		...
		var onSuccess = function (result, context) {
			context._onUrl(result);
		};
		var onError = function (result, context) {
			context._onError(result);
		};

		this._webService.RequestThumbnail_v2(
			this._assetID,
			this._assetVersionID,
			width,
			height,
			this._fileExtension,
			this._thumbnailFormatID,
			this._ov,
			onSuccess,
			onError,
			this,
		...
		$(function () {
			$(document).on('click', 'div.MediaPlayerIcon', function (e) {
				...
				if (Roblox.Thumbs.AssetImage.mediaPlayer == null) {
					Roblox.Thumbs.AssetImage.mediaPlayer = $("<div id='MediaPlayerSingleton'></div>")
						.appendTo('body')
						.jPlayer({
					...
					// Otherwise, play or pause normally.
