	...
	(Roblox.iFrameLogin = new (function () {
		function n() {
			...
			var c = function (n) {
				...
				n = function (n, t) {
					(s = !1), c(!1), t ? n.css({ 'background-color': '#FDD' }) : n.css({ 'background-color': 'white' });
				},
				l = function () {
					var w;
					if (y()) return !1;
					...
					var b = $('#Password').val(),
						...
						p = '',
						a = '';
					if (e && ((p = $('#recaptcha_challenge_field').val()), (a = $('#recaptcha_response_field').val()), p == '' || a == ''))
						return n($('#recaptcha_response_field'), !0), !1;
					e && $('#Captcha_upBadCaptcha').text(''),
						(w = onError = function (h) {
							if (h.IsValid) {
								var y;
								(y = v ? $(document.body).data('parent-url') : window.parent.location.href),
								...
								return h.ErrorCode.indexOf(t) != -1
									...
									: (h.ErrorCode.indexOf(r) != -1 &&
											(window.parent.location = '/login/twofactorauth?username=' + encodeURIComponent(l)),
									  h.ErrorCode.indexOf(i) != -1
											? (n($('#Password'), !0), $('#NotAMemberLink').hide(), $('#ForgotPasswordLink').show())
				...
				a = function () {
					...
					(t = t.replace(/ /g, '')),
						...
						t != '' && $.ajax({ type: 'GET', url: '/UserCheck/doesusernameexist?username=' + t, success: i, error: onError });
				};
