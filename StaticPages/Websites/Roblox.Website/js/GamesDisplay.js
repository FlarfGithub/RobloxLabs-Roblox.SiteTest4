...
function CreateGamesDisplay() {
	...
	function _GenerateGameObject(game) {
		...
		template
			.find('.roblox-thumbnail')
			.attr('href', game.NavigateUrl)
			.html("<img src='" + game.ThumbnailUrl + "'/>");
		...
		if (game.Purchase) if (game.Purchase.ForSale) template.find('.robux-price').addClass('robux').text(game.Purchase.Price);
		...
		if (game.BCOverlay.Url != null) {
			template
				.find('.AlwaysShown')
				.append('<img class="BCOverlay" src="' + game.BCOverlay.Url + '" alt="' + game.BCOverlay.Alt + '" />');
		}
		...
		if (game.PersonalPlaceOverlay.Url != null) {
			template
				.find('.AlwaysShown')
				.append(
					'<img class="MegaOverlay" src="' + game.PersonalPlaceOverlay.Url + '" alt="' + game.PersonalPlaceOverlay.Alt + '" />',
				);
		} else if (game.MegaOverlay.Url != null) {
			template
				.find('.AlwaysShown')
				.append('<img class="MegaOverlay" src="' + game.MegaOverlay.Url + '" alt="' + game.MegaOverlay.Alt + '" />');
		}
		...
		template.find('.AlwaysShown').hover(
			function () {
				...
				if (playerCount.attr('noplayers') === 'true') playerCount.show();
			...
			function () {
				...
				var playerCount = $(this).children('.PlayerCount');
				if (playerCount.attr('noplayers') === 'true') playerCount.hide();
				$(this).siblings('.HoverShown').hide();
			},
		);

		// Bind Click Tracking
		template.find('.GameClick').click(function () {
			$(this).append(
				'<img src="/Games/LogGameClick?filter=' +
					p._sortType[p._gamesType] +
					'&cachebuster=' +
					$.now() +
					'" width="1" height="1"/>',
			);
	...
	function _RenderCurrentView() {
		...
		if (pageNumCounter != null) $(pageNumCounter).html(_GetTotalPages(comboKey)); // bind our total page count
		...
		if (_ResultsAreInDataStore(dataAccessKey)) {
			...
			for (
				var i = 0;
				i < data.length;
				i++ // should always be 20, but possibly less with paging / data corruption
			) {
		...
		html.append(
			'<img src="/Games/LogGameImpression?filter=' +
				CreateGamesDisplay.prototype._sortType[CreateGamesDisplay.prototype._gamesType] +
				'&cachebuster=' +
				$.now() +
				'" width="1" height="1"/>',
		);
	...
	function _UpdatePagingControls() {
		...
		if (totalPages <= 1 && _renderTo != '#BCOnlyGamesContent') {
			...
			if (totalPages <= pageNum) $(_renderTo + 'NextNavButton').addClass('disabled');
			else $(_renderTo + 'NextNavButton').removeClass('disabled');

			if (pageNum == 1) $(_renderTo + 'PrevNavButton').addClass('disabled');
			else $(_renderTo + 'PrevNavButton').removeClass('disabled');
		}
	}
	/** Public **/
	...
	this.UpdateAddress = function () {
		...
		if (currSort == p._defaultSort) $.address.parameter('m', '');
		else if (currSort == 'Popular' || currSort == 'MostPopular')
			// Backwards compatible with MostPopular
			$.address.parameter('m', 'MostPopular');
		else $.address.parameter('m', currSort);
		$.address.parameter('t', p._filterType);
		var currPageNum = _GetCurrentPageNum();
		if (currPageNum == 1) $.address.parameter('p', '');
		else $.address.parameter('p', currPageNum);
	...
	this.ShowGames = function () {
		...
		if (sortType == 'RecentlyVisited') resultController = 'GetRecentlyVisitedPlaces.ashx';
	...
	this.LoadFromUrl = function () {
		...
		if ($.address.parameter('g') != null) genreType = $.address.parameter('g');
		else if (getParameterByName('g') != '') genreType = getParameterByName('g');
		else if ($.address.path() != '/') genreType = $.address.path().replace('/', '').replace('-games', '');
		else {
		...
		var filterType = 'Now';
		if ($.address.parameter('t') != null) filterType = $.address.parameter('t');
		else if (getParameterByName('t') != '') filterType = getParameterByName('t');

		var sortType = this._defaultSort;
		if ($.address.parameter('m') != null) sortType = $.address.parameter('m');
		else if (getParameterByName('m') != '') sortType = getParameterByName('m');

		if ($.inArray(sortType, CreateGamesDisplay.prototype._possibleSortTypes) == -1)
			// Backwards compatibility for "RecentlyUpdated"
			sortType = this._defaultSort;

		var pageNum = 1;
		if ($.address.parameter('p') != null) pageNum = parseInt($.address.parameter('p'));
		else if (getParameterByName('p') != '') pageNum = parseInt(getParameterByName('p'));
	...
	this.ChangePage = function (direction) {
		...
		if (_minBCLevel == 'None') {
			// Only refresh ads on non bc-only place paging
...
CreateGamesDisplay.prototype._SetCurrentCombo = function (gamesType, sortType, filterType, genreType, initialLoad) {
	...
	if (p._filterType != filterType) {
		// This has to come before Sort because of TopFavorites not allowing "Now" filter
		hasChanged = true;
		p._filterType = filterType;
		//Toggle the link to bold (remove old selected link)
		$('.SelectedFilter').removeClass('SelectedFilter');
		$('#Timespan')
			.find('[filter="' + p._filterType + '"]')
	...
	if (p._sortType[gamesType] != sortType || gamesTypeChanged || initialLoad) {
		...
		if (sortType == 'TopFavorites') {
			...
			$('#Genres')
				.find('[genre="' + p._genreType + '"]')
				.addClass('SelectedGenre');
	...
	if (gamesType == 'Build') {
		...
		$('#Sort')
			.find('[sort="' + p._sortType[gamesType] + '"]')
			.removeClass('DisabledFilter')
			.addClass('SelectedSort')
			.removeAttr('disabled');
...
CreateGamesDisplay.prototype._sortType = {
	Play: CreateGamesDisplay.prototype._defaultSort,
	Build: CreateGamesDisplay.prototype._defaultSort,
}; // Associative array for currently selected tabs
CreateGamesDisplay.prototype._filterType = 'Now'; // Possibles: Now, PastDay, PastWeek, AllTime
CreateGamesDisplay.prototype._genreType = 'all'; // Possibles: All, TownandCity, Fantasy, etc...
