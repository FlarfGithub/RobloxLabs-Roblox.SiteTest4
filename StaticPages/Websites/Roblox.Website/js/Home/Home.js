...
$(function () {
	function t() {
		...
		$('#shareButton').hide(),
			$('#loadingImage').show(),
			$.post(
				$('#HomeContainer').data('update-status-url'),
				{
		...
		function (n) {
			if (n.Count > 0) {
				$(n.Items).each(function (n, t) {
					...
					i.find('.recent-place-thumb').append(
						'<img src="' + t.ThumbnailUrl + '" alt="' + t.Name.escapeHTML() + '" class="recent-place-thumb-img" />',
					);
					i.find('.recent-place-name').append(
						'<a href="' + t.NavigateUrl + '">' + fitStringToWidth(t.Name, 200, 'recent-place-name') + ' </a>',
					);
					t.PersonalPlaceOverlay.Url != null &&
						i
							.find('.recent-place-name')
							.append(
								'<img border="0" alt="Personal Server" src="/images/icons/personal_server.png" class="recent-place-thumb-ps-overlay" />',
							);
