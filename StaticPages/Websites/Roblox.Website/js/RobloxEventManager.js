...
RobloxEventManager = new (function () {
	...
	this.triggerEvent = function (eventName, args) {
		if (this.initialized) {
			if (this.enabled) {
				...
				if (args.guid != -1) $(document).trigger(eventName, [args]);
			}
		} else {
	...
	this.startMonitor = function () {
		...
		if ($('#PlaceLauncherStatusPanel').data('is-protocol-handler-launch-enabled') == 'True') return;
