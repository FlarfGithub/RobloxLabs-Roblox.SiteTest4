...
(function () {
	...
	jQuery.color.parse = function (E) {
		...
		if (
			(D = /rgba\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(
				E,
			))
		) {
...
(function ($) {
	function Plot(placeholder, data_, options_, plugins) {
		...
		plot.pointOffset = function (point) {
			return {
				left: parseInt(axisSpecToRealAxis(point, 'xaxis').p2c(+point.x) + plotOffset.left),
		...
		function parseOptions(opts) {
			...
			if (options.shadowSize) options.series.shadowSize = options.shadowSize;
		...
		function axisSpecToRealAxis(obj, attr) {
			var a = obj[attr];
			if (!a || a == 1) return axes[attr];
			if (typeof a == 'number') return axes[attr.charAt(0) + a + attr.slice(1)];
		...
		function fillInSeriesOptions() {
			...
			for (i = 0; i < series.length; ++i) {
				...
				if (sc != null) {
					...
					else usedColors.push($.color.parse(series[i].color));
				}
			}
			...
			while (colors.length < neededColors) {
				...
				else c = $.color.parse(options.colors[i]);
		...
		function processData() {
			...
			function updateAxis(axis, min, max) {
				if (min < axis.datamin) axis.datamin = min;
				if (max > axis.datamax) axis.datamax = max;
			}
			...
			for (i = 0; i < series.length; ++i) {
				...
				if (!format) {
					...
					if (s.bars.show) format.push({ y: true, number: true, required: false, defaultValue: 0 });
				...
				if (s.datapoints.pointsize == null) s.datapoints.pointsize = format.length;
			...
			for (axis in axes) {
				if (axes[axis].datamin == topSentry) axes[axis].datamin = null;
				if (axes[axis].datamax == bottomSentry) axes[axis].datamax = null;
			}
		}
		...
		function setupGrid() {
			...
			function measureLabels(axis, axisOptions) {
				...
				if (axis == axes.xaxis || axis == axes.x2axis) {
					...
					if (axis.labelWidth == null) axis.labelWidth = canvasWidth / (axis.ticks.length > 0 ? axis.ticks.length : 1);
					...
					if (axis.labelHeight == null) {
						...
						for (i = 0; i < axis.ticks.length; ++i) {
							...
							if (l) labels.push('<div class="tickLabel" style="float:left;width:' + axis.labelWidth + 'px">' + l + '</div>');
						}
					...
					if (labels.length > 0) {
						var dummyDiv = $(
							'<div style="position:absolute;top:-10000px;font-size:smaller">' + labels.join('') + '</div>',
						).appendTo(placeholder);
						if (axis.labelWidth == null) axis.labelWidth = dummyDiv.width();
						if (axis.labelHeight == null) axis.labelHeight = dummyDiv.find('div').height();
						dummyDiv.remove();
					}
				}

				if (axis.labelWidth == null) axis.labelWidth = 0;
				if (axis.labelHeight == null) axis.labelHeight = 0;
			}
			...
			function setGridSpacing() {
				...
				if (axes.xaxis.labelHeight > 0) plotOffset.bottom = Math.max(maxOutset, axes.xaxis.labelHeight + margin);
				if (axes.yaxis.labelWidth > 0) plotOffset.left = Math.max(maxOutset, axes.yaxis.labelWidth + margin);
				if (axes.x2axis.labelHeight > 0) plotOffset.top = Math.max(maxOutset, axes.x2axis.labelHeight + margin);
				if (axes.y2axis.labelWidth > 0) plotOffset.right = Math.max(maxOutset, axes.y2axis.labelWidth + margin);
			...
			var axis;
			for (axis in axes) setRange(axes[axis], options[axis]);
			...
			for (axis in axes) setTransformationHelpers(axes[axis], options[axis]);

			if (options.grid.show) insertLabels();
		...
		function setRange(axis, axisOptions) {
			...
			if (delta == 0.0) {
				...
				if (axisOptions.min == null) min -= widen;
				// alway widen max if we couldn't widen min to ensure we
				// don't fall into min == max which doesn't work
				if (axisOptions.max == null || axisOptions.min != null) max += widen;
			} else {
		...
		function prepareTickGeneration(axis, axisOptions) {
			...
			var noTicks;
			if (typeof axisOptions.ticks == 'number' && axisOptions.ticks > 0) noTicks = axisOptions.ticks;
			else if (axis == axes.xaxis || axis == axes.x2axis)
				// heuristic based on the model a*sqrt(x) fitted to
				// some reasonable data points
				noTicks = 0.3 * Math.sqrt(canvasWidth);
			else noTicks = 0.3 * Math.sqrt(canvasHeight);
			...
			if (axisOptions.mode == 'time') {
				...
				if (axisOptions.minTickSize != null) {
					if (typeof axisOptions.tickSize == 'number') minSize = axisOptions.tickSize;
					else minSize = axisOptions.minTickSize[0] * timeUnitSize[axisOptions.minTickSize[1]];
				}

				for (i = 0; i < spec.length - 1; ++i)
					if (
						delta < (spec[i][0] * timeUnitSize[spec[i][1]] + spec[i + 1][0] * timeUnitSize[spec[i + 1][1]]) / 2 &&
				...
				formatter = function (v, axis) {
					...
					if (axisOptions.timeformat != null) return $.plot.formatDate(d, axisOptions.timeformat, axisOptions.monthNames);

					var t = axis.tickSize[0] * timeUnitSize[axis.tickSize[1]];
					var span = axis.max - axis.min;
					var suffix = axisOptions.twelveHourClock ? ' %p' : '';

					if (t < timeUnitSize.minute) fmt = '%h:%M:%S' + suffix;
					else if (t < timeUnitSize.day) {
						if (span < 2 * timeUnitSize.day) fmt = '%h:%M' + suffix;
						else fmt = '%b %d %h:%M' + suffix;
				...
				if (axisOptions.minTickSize != null && size < axisOptions.minTickSize) size = axisOptions.minTickSize;

				if (axisOptions.tickSize != null) size = axisOptions.tickSize;
		...
		function setTicks(axis, axisOptions) {
			...
			if (axisOptions.ticks == null) axis.ticks = axis.tickGenerator(axis);
			...
			if (axisOptions.autoscaleMargin != null && axis.ticks.length > 0) {
				// snap to ticks
				if (axisOptions.min == null) axis.min = Math.min(axis.min, axis.ticks[0].v);
				if (axisOptions.max == null && axis.ticks.length > 1) axis.max = Math.max(axis.max, axis.ticks[axis.ticks.length - 1].v);
			}
		}
		...
		function extractRange(ranges, coord) {
			...
			if (from != null && to != null && from > to) return { from: to, to: from, axis: axis };
		...
		function drawGrid() {
			...
			for (i = 0; i < axis.ticks.length; ++i) {
				v = axis.ticks[i].v;
				if (v <= axis.min || v >= axes.xaxis.max) continue; // skip those lying on the axes
		...
		function insertLabels() {
			...
			addLabels(axes.xaxis, function (tick, axis) {
				return (
					...
					'px;left:' +
					Math.round(plotOffset.left + axis.p2c(tick.v) - axis.labelWidth / 2) +
					'px;width:' +
					axis.labelWidth +
			...
			addLabels(axes.yaxis, function (tick, axis) {
				return (
					'<div style="position:absolute;top:' +
					Math.round(plotOffset.top + axis.p2c(tick.v) - axis.labelHeight / 2) +
					'px;right:' +
					(plotOffset.right + plotWidth + margin) +
					'px;width:' +
					axis.labelWidth +
			...
			addLabels(axes.y2axis, function (tick, axis) {
				return (
					...
					Math.round(plotOffset.top + axis.p2c(tick.v) - axis.labelHeight / 2) +
					'px;left:' +
					(plotOffset.left + plotWidth + margin) +
					'px;width:' +
					axis.labelWidth +
					'px;text-align:left" class="tickLabel">' +
					tick.label +
					'</div>'
				);
			});
		...
		function drawSeriesPoints(series) {
			...
			if (lw > 0 && sw > 0) {
				...
				ctx.lineWidth = w;
				ctx.strokeStyle = 'rgba(0,0,0,0.1)';
				plotPoints(series.datapoints, radius, null, w + w / 2, Math.PI, series.xaxis, series.yaxis);

				ctx.strokeStyle = 'rgba(0,0,0,0.2)';
				plotPoints(series.datapoints, radius, null, w / 2, Math.PI, series.xaxis, series.yaxis);
			}

			ctx.lineWidth = lw;
			ctx.strokeStyle = series.color;
			plotPoints(series.datapoints, radius, getFillStyle(series.points, series.color), 0, 2 * Math.PI, series.xaxis, series.yaxis);
			ctx.restore();
		}
		...
		function triggerClickHoverEvent(eventname, event, seriesFilter) {
			...
			if (axes.xaxis.used) pos.x = axes.xaxis.c2p(canvasX);
			if (axes.yaxis.used) pos.y = axes.yaxis.c2p(canvasY);
			if (axes.x2axis.used) pos.x2 = axes.x2axis.c2p(canvasX);
			if (axes.y2axis.used) pos.y2 = axes.y2axis.c2p(canvasY);
			...
			if (options.grid.autoHighlight) {
				...
				for (var i = 0; i < highlights.length; ++i) {
					var h = highlights[i];
					if (h.auto == eventname && !(item && h.series == item.series && h.point == item.datapoint))
